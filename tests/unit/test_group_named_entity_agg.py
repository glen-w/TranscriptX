"""Offline unit coverage for named-entity group aggregation (no model runtime).

Filename avoids auto ``requires_models`` path heuristics that match ``ner``.
"""
...
from transcriptx.core.analysis.aggregation import ner as ner_aggregation
...
@pytest.mark.unit
def test_aggregate_named_entity_group_canonical_and_unidentified(
    monkeypatch: Any,
) -> None:
    segments_map = {
        "a.json": [
