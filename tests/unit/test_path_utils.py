...
def test_find_enriched_transcript_only_standard_layout(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(_path_core, "OUTPUTS_DIR", str(tmp_path))
    transcript_path = os.path.join(str(tmp_path), "meeting.json")
    Path(transcript_path).touch()
    canonical = _path_core.get_enriched_transcript_path(transcript_path, "sentiment")
    os.makedirs(os.path.dirname(canonical), exist_ok=True)
    Path(canonical).touch()
    assert (
        _path_core.find_enriched_transcript(transcript_path, "sentiment") == canonical
    )

    legacy_only_json = str(tmp_path / "oldlayout.json")
    Path(legacy_only_json).touch()
    legacy_path = os.path.join(
        str(tmp_path),
        "oldlayout",
        "sentiment",
        "oldlayout_with_sentiment.json",
    )
    os.makedirs(os.path.dirname(legacy_path), exist_ok=True)
    Path(legacy_path).touch()
    assert _path_core.find_enriched_transcript(legacy_only_json, "sentiment") is None
