...
class TestConfigLifecycle:
    ...
    def test_get_config_has_required_sections(self):
        """Config has analysis, output, and logging sections (transcription removed; analysis-only)."""
        config = get_config()
...
class TestTranscriptLoaderHighLeverage:
    ...
    def test_load_segments_empty_list(self, tmp_path):
        """v1.0 artifact with empty segments list returns []."""
    ...
    def test_load_segments_direct_list_root_rejected(self, tmp_path):
        """Bare list JSON is not a v1.0 artifact."""
