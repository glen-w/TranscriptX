...
class TestPreflightImport:
    ...
    def test_missing_module_detection(self):
        ...
        resolved = pipeline.resolve_dependencies(["nonexistent_module_xyz", "sentiment"])
        
        # The missing module should not be in resolved (it's filtered out)
        assert "nonexistent_module_xyz" not in resolved, "Missing module should not be resolved"
        ...
        assert "sentiment" in resolved or len(resolved) > 0, "Valid module should be resolved"
...
class TestDeterministicOrdering:
            ...
            def normalize_for_hash(data):
                ...
                if isinstance(data, dict):
                    return {
                        ...
                        if k not in ["execution_time", "timestamp", "start_time", "end_time"]
                    }
    ...
    def test_module_ordering_independent_of_registration_order(self):
        ...
        entity_sentiment_pos = order.index("entity_sentiment") if "entity_sentiment" in order else -1
        
        if ner_pos >= 0 and entity_sentiment_pos >= 0:
            assert ner_pos < entity_sentiment_pos, "ner should come before entity_sentiment"
        if sentiment_pos >= 0 and entity_sentiment_pos >= 0:
            assert sentiment_pos < entity_sentiment_pos, "sentiment should come before entity_sentiment"
...
class TestParallelExecutionFrozenContext:
    ...
    def test_parallel_execution_frozen_context(self, tmp_path):
        ...
        transcript_file.write_text(json.dumps({
            "segments": [{"start": 0.0, "end": 1.0, "speaker": "Alice", "speaker_db_id": 1, "text": "Test"}]
        }))
