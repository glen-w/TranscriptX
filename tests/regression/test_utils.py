...
including trace capture, subprocess helpers, output hashing, and
...
def capture_resolution_trace(
    ...
    # Get final result
    try:
        final_result = resolver.resolve_with_result(
            file_path, file_type, validate_state
        )
...
def run_cli_with_capture(
    ...
    """
    Run ``python -m transcriptx.web`` with extra argv and capture stdout/stderr.

    Args:
        command: First argv after the module (often unused; launcher only supports --host/--port)
...
def compare_execution_plans(
    plan1: Dict[str, Any], plan2: Dict[str, Any]
) -> Dict[str, Any]:
    """
