...
class TestCorruptJSONRecovery:
    ...
    def test_corrupt_json_auto_recover(self, fixture_corrupt_json, monkeypatch):
        ...
        from transcriptx.core.utils.state_backup import BACKUP_DIR
        backup_dir = state_file.parent / "backups" / "processing_state"
        backup_dir.mkdir(parents=True, exist_ok=True)
        
        # Monkeypatch BACKUP_DIR to point to our test directory
        monkeypatch.setattr("transcriptx.core.utils.state_backup.BACKUP_DIR", backup_dir)
...
class TestConcurrentWriters:
    ...
    def test_concurrent_writers_one_waits(self, fixture_concurrent_writers):
        ...
        with multiprocessing.Pool(processes=2) as pool:
            results = pool.starmap(
                ...
                [(str(state_file), i, i * 0.1) for i in range(2)]
            )
    ...
    def test_concurrent_writers_no_race_condition(
        self, fixture_concurrent_writers
    ):
        ...
        with multiprocessing.Pool(processes=3) as pool:
            results = pool.starmap(
                self._writer_process,
                [(str(state_file), i, i * 0.05) for i in range(3)]
            )
