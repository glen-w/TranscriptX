...
def test_no_whisperx_in_core():
    """No 'whisperx' string in core/** (format adapters live under io/)."""
    base = os.path.join(os.path.dirname(__file__), "..", "..", "src", "transcriptx")
    exclude = frozenset({"__pycache__", ".mypy_cache"})
    root = os.path.join(base, "core")
...
def test_no_ghost_nouns_in_source():
    """No ghost nouns (docker socket, hf_token, diarised transcripts dir) in core/."""
    base = os.path.join(os.path.dirname(__file__), "..", "..", "src", "transcriptx")
    exclude = frozenset({"__pycache__", ".mypy_cache"})
    patterns = [
...
def test_compose_has_cache_volume():
    """docker-compose.yml must define transcriptx_cache volume for the web service."""
    repo_root = os.path.join(os.path.dirname(__file__), "..", "..")
    compose_path = os.path.join(repo_root, "docker-compose.yml")
    if not os.path.isfile(compose_path):
        pytest.skip("docker-compose.yml not found")
...
def test_default_compose_downloads_enabled():
    """docker-compose should default downloads to enabled for web service."""
    repo_root = os.path.join(os.path.dirname(__file__), "..", "..")
    compose_path = os.path.join(repo_root, "docker-compose.yml")
    if not os.path.isfile(compose_path):
        pytest.skip("docker-compose.yml not found")
    ...
    assert (
        "TRANSCRIPTX_DISABLE_DOWNLOADS=${TRANSCRIPTX_DISABLE_DOWNLOADS:-0}" in content
    ), "compose default should enable downloads unless explicitly disabled"
