...
]  # reason: patches load_processing_state/processed_count which were removed/renamed; owner: cli; remove_by: when batch_wav API stabilizes
...
class TestBatchWAVWorkflowImpl:
    ...
    def test_batch_workflow_all_files_processed(
        self, mock_confirm, mock_filter_new, mock_discover, mock_select_folder, tmp_path
    ):
...
class TestBatchProcessFiles:
    ...
    def test_batch_process_files_success(
        ...
        wav_files = [tmp_path / "file1.wav", tmp_path / "file2.wav"]
        for f in wav_files:
    ...
    @patch("transcriptx.cli.batch_wav_workflow.resume_batch_processing")
    @patch("transcriptx.cli.batch_wav_workflow.questionary.confirm")
    def test_batch_process_files_resume(self, mock_confirm, mock_resume, tmp_path):
        ...
        with (
            patch(
                "transcriptx.cli.batch_wav_workflow.check_whisperx_compose_service",
                return_value=True,
            ),
            patch(
                "transcriptx.cli.batch_wav_workflow.process_single_file"
            ) as mock_process,
...
class TestShowSummary:
    ...
    def test_show_summary_with_failures(self, tmp_path):
        ...
        results = {
            "total_files": 2,
            "successful": [],
            "failed": [{"file": str(tmp_path / "file1.wav"), "error": "Test error"}],
            "skipped": [],
        }
...
class TestBatchWorkflowErrorHandling:
    ...
    def test_individual_file_failure_isolation(
        self, mock_process, mock_filter, mock_discover, mock_select, tmp_path
    ):
        ...
        with (
            patch(
                "transcriptx.cli.batch_wav_workflow.questionary.select"
            ) as mock_select_q,
            patch("transcriptx.cli.batch_wav_workflow._show_summary"),
        ):
    ...
    @patch("transcriptx.cli.batch_wav_workflow.select_folder_interactive")
    @patch("transcriptx.cli.batch_wav_workflow.discover_wav_files")
    def test_file_discovery_error(self, mock_discover, mock_select, tmp_path):
    ...
    def test_state_file_corruption(
        self, mock_load_state, mock_filter, mock_discover, mock_select, tmp_path
    ):
        ...
        with patch(
            "transcriptx.cli.batch_wav_workflow.filter_new_files"
        ) as mock_filter_new:
    ...
    def test_state_file_locked(
        ...
        mock_select.return_value = tmp_path
        mock_discover.return_value = wav_files
        mock_load_state.side_effect = BlockingIOError(
    ...
    @patch("transcriptx.cli.batch_wav_workflow.batch_process_files")
    def test_memory_exhaustion_during_batch(self, mock_process, tmp_path):
    ...
    @patch("transcriptx.cli.batch_wav_workflow.resume_batch_processing")
    @patch("transcriptx.cli.batch_wav_workflow.load_processing_state")
    def test_resume_with_corrupted_state(self, mock_load_state, mock_resume, tmp_path):
    ...
    @patch("transcriptx.cli.batch_wav_workflow.resume_batch_processing")
    def test_resume_with_missing_files(self, mock_resume, tmp_path):
        ...
        with (
            patch(
                "transcriptx.cli.batch_wav_workflow.questionary.confirm",
    ...
    @patch("transcriptx.cli.batch_wav_workflow.resume_batch_processing")
    def test_resume_with_changed_file_structure(self, mock_resume, tmp_path):
        """Test resume when file structure has changed."""
        wav_files = [tmp_path / "file1.wav"]
        wav_files[0].write_bytes(b"fake")

        mock_resume.return_value = {"can_resume": True, "remaining_files": wav_files}
        ...
        with (
            ...
            patch(
                "transcriptx.cli.batch_wav_workflow.discover_wav_files"
            ) as mock_discover,
            patch(
                "transcriptx.cli.batch_wav_workflow.process_single_file"
            ) as mock_process,
    ...
    @patch("transcriptx.cli.batch_wav_workflow.batch_process_files")
    def test_disk_space_exhaustion(self, mock_process, tmp_path):
...
class TestAssessAndSuggestPreprocessing:
    ...
    def test_assess_global_mode_off(
        self, mock_progress, mock_confirm, mock_assess, tmp_path
    ):
    ...
    @patch("transcriptx.cli.batch_wav_workflow.questionary.confirm")
    @patch("transcriptx.cli.batch_wav_workflow.Progress")
    def test_assess_global_mode_auto(
        self, mock_progress, mock_confirm, mock_assess, tmp_path
    ):
    ...
    @patch("transcriptx.cli.batch_wav_workflow.questionary.confirm")
    @patch("transcriptx.cli.batch_wav_workflow.Progress")
    def test_assess_global_mode_suggest(
        self, mock_progress, mock_confirm, mock_assess, tmp_path
    ):
    ...
    def test_assess_skip_confirm(
        ...
        decisions = _assess_and_suggest_preprocessing(
            wav_files, config, skip_confirm=True
        )
    ...
    @patch("transcriptx.cli.batch_wav_workflow.questionary.confirm")
    @patch("transcriptx.cli.batch_wav_workflow.Progress")
    def test_assess_selected_mode_per_step_suggest(
        self, mock_progress, mock_confirm, mock_assess, tmp_path
    ):
