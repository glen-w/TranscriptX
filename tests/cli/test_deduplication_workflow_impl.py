...
class TestDeduplicationWorkflowImpl:
    ...
    @patch("transcriptx.cli.interactive_menu._check_and_install_librosa")
    @patch(
        "transcriptx.cli.deduplication_workflow.find_duplicate_files_by_size_and_content"
    )
    @patch("transcriptx.cli.deduplication_workflow.find_duplicate_files_by_size")
    def test_deduplication_workflow_no_duplicates(
        self,
        mock_find_by_size,
        mock_find_by_content,
        mock_check_librosa,
        mock_librosa_available,
        mock_discover,
        mock_select_folder,
        tmp_path,
    ):
    ...
    def test_deduplication_workflow_with_duplicates(
        ...
        mock_librosa_available.return_value = False  # Use size-only detection
        mock_find_by_size.return_value = {13: [file1, file2]}  # Same size
...
class TestPrecomputeFileMetadata:
    ...
    def test_precompute_metadata_regular_files(self, tmp_path):
        ...
        with patch(
            "transcriptx.cli.deduplication_workflow._is_audio_file", return_value=False
        ):
            metadata = _precompute_file_metadata(files)
