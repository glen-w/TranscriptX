...
]  # reason: patches select_audio_file/validate_transcript_file/start_whisperx_compose_service; API uses target not transcript_path; owner: cli; remove_by: when transcription_workflow API stabilizes
...
class TestTranscriptionWorkflowImpl:
    ...
    def test_transcription_workflow_no_analysis(
        self, mock_confirm, mock_transcribe, mock_select_audio, tmp_path, mock_config
    ):
...
class TestTranscriptionWorkflowErrorHandling:
    ...
    def test_whisperx_service_crash_during_transcription(
        ...
        with patch(
            "transcriptx.cli.transcription_workflow.log_error"
        ) as mock_log_error:
            _run_transcription_workflow_impl()
    ...
    @patch("transcriptx.cli.transcription_workflow.transcribe_with_whisperx")
    def test_transcription_timeout(self, mock_transcribe, mock_select_audio, tmp_path):
    ...
    def test_transcription_returns_malformed_data(
        ...
        with (
            patch(
                "transcriptx.cli.transcription_workflow.validate_transcript_file"
            ) as mock_validate,
            patch("transcriptx.cli.transcription_workflow.log_error") as mock_log_error,
        ):
    ...
    @patch("transcriptx.cli.transcription_workflow.transcribe_with_whisperx")
    def test_file_lock_conflict(self, mock_transcribe, mock_select_audio, tmp_path):
        """Test handling when file is locked."""
        audio_file = tmp_path / "test.mp3"
        audio_file.write_bytes(b"fake audio")

        mock_select_audio.return_value = audio_file
        mock_transcribe.side_effect = BlockingIOError(
