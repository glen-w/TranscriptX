...
]  # reason: patches removed/renamed CLI APIs (select_transcript_file_interactive, generate_stats_from_file, validate_transcript_file); owner: cli; remove_by: when CLI stabilizes
...
class TestPostAnalysisMenu:
    ...
    def test_post_analysis_menu_open_html(
        ...
        with patch(
            "transcriptx.cli.analysis_workflow.get_html_summary_path"
        ) as mock_path:
...
class TestAnalysisWorkflowErrorHandling:
    ...
    def test_invalid_transcript_file_missing_fields(
        ...
        with patch("transcriptx.cli.analysis_workflow.log_error"):
            _run_analysis_workflow_impl()
    ...
    def test_module_execution_failure(
        ...
        with (
            patch(
                "transcriptx.cli.analysis_workflow.questionary.confirm"
            ) as mock_confirm,
            patch(
                "transcriptx.cli.analysis_workflow.filter_modules_by_mode"
            ) as mock_filter,
        ):
            mock_confirm.return_value.ask.return_value = True
    ...
    def test_output_generation_failure_disk_full(
        ...
        with (
            patch(
                "transcriptx.cli.analysis_workflow.select_analysis_mode"
            ) as mock_mode,
            patch(
                "transcriptx.cli.analysis_workflow.select_analysis_modules"
            ) as mock_modules,
            patch(
                "transcriptx.cli.analysis_workflow.questionary.confirm"
            ) as mock_confirm,
    ...
    def test_wrong_file_format(self, mock_select, tmp_path):
        ...
        with (
            patch(
                "transcriptx.cli.analysis_workflow.validate_transcript_file"
            ) as mock_validate,
