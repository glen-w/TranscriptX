...
class TestCheckFFmpegAvailable:
    ...
    @patch("transcriptx.cli.audio.tools._find_ffmpeg_path")
    @patch("transcriptx.cli.audio.tools.subprocess.run")
...
class TestCheckFFplayAvailable:
    ...
    @patch("transcriptx.cli.audio.tools.subprocess.run")
    @patch("transcriptx.cli.audio.tools.shutil.which")
    def test_ffplay_available(self, mock_which, mock_run):
    ...
    @patch("transcriptx.cli.audio.tools.subprocess.run")
    @patch("transcriptx.cli.audio.tools.os.path.exists", return_value=False)
    @patch("transcriptx.cli.audio.tools.shutil.which")
    def test_ffplay_not_available(self, mock_which, mock_exists, mock_run):
...
class TestConvertWAVToMP3:
    ...
    @patch(
        "transcriptx.cli.audio.conversion.check_ffmpeg_available",
        return_value=(True, None),
    )
    @patch("transcriptx.cli.audio.conversion.apply_preprocessing")
    @patch("transcriptx.cli.audio.conversion.PYDUB_AVAILABLE", True)
    @patch("transcriptx.cli.audio.conversion.AudioSegment")
    def test_convert_wav_to_mp3_success(
        self, mock_audio_segment, mock_apply_preprocessing, _mock_ffmpeg, tmp_path
    ):
        ...
        def _write_output(path, **kwargs):
            Path(path).parent.mkdir(parents=True, exist_ok=True)
            Path(path).write_bytes(b"")

        mock_segment.export.side_effect = _write_output
        mock_apply_preprocessing.return_value = (mock_segment, [])
    ...
    @patch("transcriptx.cli.audio.conversion.PYDUB_AVAILABLE", False)
    ...
    @patch("transcriptx.cli.audio.conversion.PYDUB_AVAILABLE", True)
    @patch("transcriptx.cli.audio.conversion.AudioSegment")
    def test_convert_wav_to_mp3_decode_error(
        self, mock_audio_segment, mock_apply_preprocessing, _mock_ffmpeg, tmp_path
    ):
        ...
        from transcriptx.cli.audio.conversion import CouldntDecodeError

        mock_audio_segment.from_file.side_effect = CouldntDecodeError("Cannot decode")
...
class TestAssessAudioNoise:
    ...
    @patch("transcriptx.cli.audio.preprocessing.PYDUB_AVAILABLE", True)
    @patch("transcriptx.cli.audio.preprocessing.AudioSegment")
    ...
    @patch("transcriptx.cli.audio.preprocessing.PYDUB_AVAILABLE", False)
    ...
    def test_assess_audio_noise_stereo(self, mock_audio_segment, tmp_path):
        ...
        assert (
            "mono" in assessment["suggested_steps"]
            or "resample" in assessment["suggested_steps"]
        )
...
class TestCheckAudioCompliance:
    ...
    @patch("transcriptx.cli.audio.preprocessing.PYDUB_AVAILABLE", True)
    @patch("transcriptx.cli.audio.preprocessing.AudioSegment")
    def test_check_audio_compliance_wrong_sample_rate(
        self, mock_audio_segment, tmp_path
    ):
...
class TestNormalizeLoudness:
    ...
    @patch("transcriptx.cli.audio.preprocessing.PYDUB_AVAILABLE", True)
    @patch("transcriptx.cli.audio.preprocessing.PYLoudnorm_AVAILABLE", False)
    @patch("transcriptx.cli.audio.preprocessing.SOUNDFILE_AVAILABLE", False)
    @patch("transcriptx.cli.audio.preprocessing.AudioSegment")
...
class TestDenoiseAudio:
    ...
    @patch("transcriptx.cli.audio.preprocessing.PYDUB_AVAILABLE", True)
    @patch("transcriptx.cli.audio.preprocessing.NOISEREDUCE_AVAILABLE", False)
    @patch("transcriptx.cli.audio.preprocessing.AudioSegment")
...
class TestGetEffectiveMode:
    ...
    def test_effective_mode_selected(self):
        ...
        from transcriptx.cli.audio.preprocessing import _get_effective_mode
...
class TestApplyPreprocessing:
    ...
    def test_apply_preprocessing_normalize(self, mock_audio_segment):
        ...
        with patch(
            "transcriptx.cli.audio.preprocessing.normalize_loudness",
    ...
    def test_apply_preprocessing_denoise(self, mock_audio_segment):
        ...
        with patch(
            "transcriptx.cli.audio.preprocessing.denoise_audio",
            return_value=mock_segment,
        ):
            result, steps = apply_preprocessing(mock_segment, config)
    ...
    def test_apply_preprocessing_suggest_mode_with_decisions(self, mock_audio_segment):
        ...
        result, steps = apply_preprocessing(
            mock_segment, config, None, preprocessing_decisions
        )
