...
class TestFormatTranscriptFileWithAnalysis:
    ...
    def test_format_transcript_with_segments(self, tmp_path):
        ...
        transcript_file.write_text(
            '{"segments": [{"speaker": "SPEAKER_00", "text": "Test"}]}'
        )
    ...
    @patch("transcriptx.cli.file_selection_utils._has_analysis_outputs")
    def test_format_transcript_speaker_map_named_shows_n_named(
        self, mock_has_analysis, tmp_path
    ):
        """Transcript with speaker_map and named speakers shows '👤 N named'."""
        ...
        formatted = _format_transcript_file_with_analysis(transcript_file)
        assert "👤 2 named" in formatted

    @patch("transcriptx.cli.file_selection_utils._has_analysis_outputs")
    def test_format_transcript_no_speaker_map_shows_0_named_no_map(
        self, mock_has_analysis, tmp_path
    ):
        """Transcript with no speaker_map and only placeholders shows '⚠️ 0 named (no map)'."""
        mock_has_analysis.return_value = True
        transcript_file = tmp_path / "test.json"
        transcript_file.write_text(
            '{"segments": [{"speaker": "SPEAKER_01", "text": "Test"}]}'
        )
        formatted = _format_transcript_file_with_analysis(transcript_file)
        assert "⚠️ 0 named" in formatted
        assert "(no map)" in formatted
        assert "map speakers to unlock analysis" in formatted

    @patch("transcriptx.cli.file_selection_utils._has_analysis_outputs")
    def test_format_transcript_map_present_but_0_named_shows_map_hint(
        self, mock_has_analysis, tmp_path
    ):
        """Transcript with speaker_map but segments not in map shows '⚠️ 0 named (map?)'."""
        mock_has_analysis.return_value = True
        transcript_file = tmp_path / "test.json"
        # Map has SPEAKER_00 -> Alice but segments use SPEAKER_01 (unmapped)
        transcript_file.write_text(
            '{"segments": [{"speaker": "SPEAKER_01", "text": "Test"}], '
...
class TestHasAnalysisOutputs:
    ...
    def test_has_analysis_outputs_true(self, tmp_path):
        ...
        with patch(
            "transcriptx.cli.file_selection_utils.get_transcript_dir"
        ) as mock_get_dir:
