...
class TestSpeakerWorkflowImpl:
    ...
    def test_speaker_workflow_complete(self, tmp_path):
        ...
        transcript_path.write_text('{"segments": [{"speaker": "SPEAKER_00", "text": "Hello"}]}')

        with patch("transcriptx.cli.speaker_workflow.select_folder_interactive") as mock_select_folder, \
             patch("transcriptx.cli.speaker_workflow.questionary.select") as mock_select, \
             patch("transcriptx.cli.speaker_workflow.load_segments") as mock_load_segments, \
             patch("transcriptx.cli.speaker_workflow.build_speaker_map") as mock_build_map, \
             patch("transcriptx.cli.speaker_workflow.rename_transcript_after_speaker_mapping") as mock_rename, \
             patch("transcriptx.cli.speaker_workflow.get_current_transcript_path_from_state") as mock_current_path, \
             patch("transcriptx.cli.speaker_workflow.store_transcript_after_speaker_identification") as mock_store:

            mock_select_folder.return_value = tmp_path
            mock_select.return_value.ask.return_value = f"✨ {transcript_path.name}"
            mock_load_segments.return_value = [{"speaker": "SPEAKER_00", "text": "Hello"}]
