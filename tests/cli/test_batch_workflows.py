...
def test_batch_analysis_all_invalid_paths(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        batch_workflows, "_resolve_transcript_path", lambda path: (_ for _ in ()).throw(FileNotFoundError())
    )
    monkeypatch.setattr(
        batch_workflows, "apply_analysis_mode_settings_non_interactive", lambda *args, **kwargs: None
    )
