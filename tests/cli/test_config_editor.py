...
class TestEditConfigInteractive:
    ...
    def test_displays_main_menu(self):
        ...
        with (
            patch("transcriptx.cli.config_editor.get_config") as mock_config,
    ...
    def test_edits_transcription_config(self):
        ...
        with (
            ...
            patch("transcriptx.cli.config_editor.questionary") as mock_q,
            patch(
                "transcriptx.cli.config_editor.edit_transcription_config"
            ) as mock_edit,
        ):
...
class TestEditAnalysisConfig:
    ...
    def test_setters_update_config(self):
        ...
        def fake_menu_loop(title, items, on_back, dirty_tracker, mark_dirty):
            target = next(
                item for item in items if item.key == "analysis.sentiment_window_size"
            )
            target.setter(25)
            mark_dirty()
...
class TestEditLoggingConfig:
    ...
    def test_edits_audio_preprocessing_config(self):
        ...
        with (
            ...
            patch("transcriptx.cli.config_editor.questionary") as mock_q,
            patch(
                "transcriptx.cli.config_editor.edit_audio_preprocessing_config"
            ) as mock_edit,
        ):
...
class TestSaveConfigInteractive:
    ...
    def test_saves_config(self, tmp_path):
        ...
        with (
            patch("transcriptx.cli.config_editors.save.questionary") as mock_q,
            patch("transcriptx.cli.config_editors.save.print"),
        ):
