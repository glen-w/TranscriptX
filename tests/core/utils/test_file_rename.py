...
class TestExtractDatePrefix:
    ...
    def test_handles_extraction_errors(self, tmp_path):
        ...
        with (
            patch(
                "transcriptx.core.utils.file_rename.extract_date_prefix_from_filename"
            ) as mock_extract,
...
class TestFindOriginalAudioFile:
    ...
    def test_finds_audio_from_processing_state(self, tmp_path, monkeypatch):
        ...
        state_data = {
            "processed_files": {str(audio_path): {"transcript_path": transcript_path}}
        }
    ...
    def test_finds_audio_from_uuid_keyed_state(self, tmp_path):
        """Test finding audio when state is keyed by UUID and path is in audio_path."""
        transcript_path = str(tmp_path / "test.json")
        audio_path = tmp_path / "recordings" / "test.wav"
    ...
    def test_uuid_key_no_audio_path_finds_via_recordings(self, tmp_path):
        """When state is UUID-keyed and has no audio_path, resolve via recordings dir before logging."""
        transcript_path = str(tmp_path / "meeting.json")
        recordings_dir = tmp_path / "recordings"
        recordings_dir.mkdir(parents=True)
        audio_path = recordings_dir / "meeting.wav"
        audio_path.write_text("fake audio")
    ...
    def test_handles_errors_gracefully(self, tmp_path, monkeypatch):
        ...
        with (
            patch(
                "transcriptx.core.utils.file_rename.PROCESSING_STATE_FILE",
                tmp_path / "nonexistent.json",
            ),
            patch("transcriptx.core.utils.file_rename.log_error"),
        ):
            result = find_original_audio_file(transcript_path)
...
class TestRenameTranscriptAfterSpeakerMapping:
    ...
    def test_renames_transcript_files(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch(
                "transcriptx.core.utils.file_rename.extract_date_prefix"
            ) as mock_prefix,
            patch(
                "transcriptx.core.utils.file_rename.prompt_for_rename"
            ) as mock_prompt,
        ):
    ...
    def test_handles_missing_audio(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch(
                "transcriptx.core.utils.file_rename.extract_date_prefix_from_transcript"
            ) as mock_transcript_prefix,
            patch(
                "transcriptx.core.utils.file_rename.prompt_for_rename"
            ) as mock_prompt,
        ):
