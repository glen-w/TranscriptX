...
class TestGetTranscriptDir:
    ...
    def test_returns_transcript_directory(self):
        ...
        assert Path(result).is_absolute() or Path(result).exists() or "outputs" in result
...
class TestPathResolutionStrategies:
    ...
    def test_exact_path_strategy(self, tmp_path):
        ...
        from transcriptx.core.utils.path_resolver import ExactPathStrategy, PathResolutionResult
    ...
    def test_state_file_strategy(self, tmp_path):
        ...
        from transcriptx.core.utils.path_resolver import StateFilePathStrategy, PathResolutionResult
        ...
        strategy = StateFilePathStrategy(state_loader=mock_state_loader, validate_paths=False)
    ...
    def test_strategy_ordering(self, tmp_path):
        ...
        from transcriptx.core.utils.path_resolver import PathResolver, ExactPathStrategy, StateFilePathStrategy
        
        test_file = tmp_path / "test.json"
        test_file.write_text("{}")
        
        # Create resolver with strategies in order
        resolver = PathResolver(strategies=[
            ExactPathStrategy(),
    ...
    def test_cache_ttl_behavior(self):
        ...
        from transcriptx.core.utils.path_utils import invalidate_path_cache, get_cache_stats
    ...
    def test_state_path_validation(self, tmp_path):
        ...
        strategy = StateFilePathStrategy(state_loader=mock_state_loader, validate_paths=True)
    ...
    def test_resolution_trace_completeness(self, tmp_path):
        ...
        resolver = PathResolver(strategies=[ExactPathStrategy()])
        trace = resolver.resolve_with_trace(str(test_file), "transcript", validate_state=False)
        ...
        if trace["final_result"] is not None:
            assert "path" in trace["final_result"] or "strategy" in trace["final_result"]
