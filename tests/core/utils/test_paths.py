...
from transcriptx.core.utils._path_cache import get_cache_stats, invalidate_path_cache
...
class TestStripDuplicateFilenameSuffix:
    """Tests for strip_duplicate_filename_suffix (Finder/Explorer copy suffixes)."""

    def test_strips_small_parenthesized_counter(self) -> None:
        assert strip_duplicate_filename_suffix("250612_CSE2 (1)") == "250612_CSE2"
        assert strip_duplicate_filename_suffix("meeting (12)") == "meeting"
        assert strip_duplicate_filename_suffix("x (999)") == "x"

    def test_does_not_strip_four_digit_year_in_parens(self) -> None:
        assert strip_duplicate_filename_suffix("session (2024)") == "session (2024)"

    def test_unmatched_stems_unchanged(self) -> None:
        assert strip_duplicate_filename_suffix("no_suffix") == "no_suffix"
        assert strip_duplicate_filename_suffix("") == ""
...
class TestPathResolution:
    ...
    def test_resolve_file_path_matches_default_resolver(self, tmp_path):
        """Façade returns the same path as PathResolver.resolve for the same inputs."""
        from transcriptx.core.utils._path_resolution import resolve_file_path
        from transcriptx.core.utils.path_resolver import get_default_resolver

        test_file = tmp_path / "parity.json"
        test_file.write_text("{}")

        r1 = resolve_file_path(
            str(test_file),
            file_type="transcript",
            validate_state=False,
            use_cache=False,
        )
        r2 = get_default_resolver().resolve(
            str(test_file),
            "transcript",
            validate_state=False,
            use_cache=False,
