...
class TestPerformanceEstimator:
    ...
    def test_performance_estimator_no_transcription_method(self, db_session_factory):
        """PerformanceEstimator must not have estimate_transcription_time (decoupled)."""
        ...
        with patch(
            ...
            estimator = PerformanceEstimator()
            assert not hasattr(estimator, "estimate_transcription_time")
            result = estimator.estimate_pipeline_time(
                modules=["sentiment"], transcript_segments=10
            )
