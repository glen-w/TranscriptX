...
class TestSaveGlobalData:
    ...
    def test_saves_global_data(self, tmp_path):
        ...
        structure = {
            "global_data_dir": str(tmp_path / "data" / "global")
        }
...
class TestSaveSpeakerData:
    ...
    def test_saves_speaker_data(self, tmp_path):
        ...
        structure = {
            "speaker_data_dir": str(tmp_path / "data" / "speakers")
        }
...
class TestOutputBuilderAdvanced:
    ...
    def output_structure(self, builder, tmp_path):
        ...
        transcript_path = str(tmp_path / "test.json")
        return builder.create_standard_output_structure(transcript_path, base_output_dir=str(tmp_path))
    ...
    def test_chart_organization(self, builder, output_structure, tmp_path):
        ...
        # Save global chart
        saved_path = builder.save_chart(str(chart_file), output_structure, chart_type="global")
        assert Path(saved_path).exists()
        assert "global" in saved_path
        
        # Save speaker chart
        saved_path = builder.save_chart(
            str(chart_file), output_structure, chart_type="speaker", speaker_id="SPEAKER_00"
        )
        assert Path(saved_path).exists()
    ...
    def test_error_handling_invalid_paths(self, builder):
        ...
        invalid_structure = {
            "global_data_dir": "/invalid/path/that/does/not/exist"
        }
