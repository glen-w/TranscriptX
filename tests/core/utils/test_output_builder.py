...
class TestOutputBuilderEdgeCases:
    """Additional output builder tests: timestamps, sanitization, chart/save edge cases."""

    def test_get_timestamp_returns_iso_format(self):
        """_get_timestamp returns a string parseable as ISO datetime."""
    ...
    def test_sanitize_filename_empty_and_whitespace(self):
        """_sanitize_filename handles empty and whitespace-only names."""
        builder = OutputStructureBuilder("test")
        assert builder._sanitize_filename("") == ""
        assert builder._sanitize_filename("   ") == ""
        assert builder._sanitize_filename(". .") == ""

    def test_sanitize_filename_unicode_preserved(self):
        """_sanitize_filename preserves safe unicode characters."""
        builder = OutputStructureBuilder("test")
        name = "speaker_ñame_日本"
        out = builder._sanitize_filename(name)
        assert "ñ" in out or "日本" in out
        assert "speaker" in out

    def test_save_chart_speaker_type_without_speaker_id_uses_basename(self, tmp_path):
        """save_chart with chart_type=speaker and speaker_id=None uses chart basename."""
        builder = OutputStructureBuilder("stats")
        structure = {
            "module_dir": str(tmp_path / "m"),
            "global_charts_dir": str(tmp_path / "m" / "charts" / "global"),
            "speaker_charts_dir": str(tmp_path / "m" / "charts" / "speakers"),
        }
        for p in structure.values():
            Path(p).mkdir(parents=True, exist_ok=True)
        chart_src = tmp_path / "report.png"
        chart_src.write_bytes(b"x")
        path = builder.save_chart(
            str(chart_src), structure, chart_type="speaker", speaker_id=None
        )
        assert Path(path).exists()
        assert path.endswith("report.png")

    def test_cleanup_empty_directories_removes_empty_leaves(self, tmp_path):
        """cleanup_empty_directories removes directories that are empty."""
        builder = OutputStructureBuilder("test")
        transcript_path = str(tmp_path / "t.json")
        structure = builder.create_standard_output_structure(
            transcript_path, base_output_dir=str(tmp_path)
        )
        # Leave global_data_dir and speaker_data_dir empty; add a file to module_dir
        (Path(structure["module_dir"]) / "summary.json").write_text("{}")
        builder.cleanup_empty_directories(structure)
        # Empty leaves in the structure should be removed
        assert not Path(structure["global_data_dir"]).exists()
        assert not Path(structure["speaker_data_dir"]).exists()

    def test_create_standard_output_structure_uses_get_transcript_dir_when_no_base(
        self, tmp_path
    ):
        """When base_output_dir is None, create_standard_output_structure uses get_transcript_dir."""
        from unittest.mock import patch

        transcript_path = str(tmp_path / "sub" / "transcript.json")
        Path(transcript_path).parent.mkdir(parents=True, exist_ok=True)
        custom_base = str(tmp_path / "outputs")
        with patch(
            "transcriptx.core.utils._path_core.get_transcript_dir",
            return_value=custom_base,
        ):
            builder = OutputStructureBuilder("stats")
            structure = builder.create_standard_output_structure(transcript_path)
        assert custom_base in structure["module_dir"]
        assert Path(structure["module_dir"]).exists()

    def test_save_speaker_data_empty_dict_returns_empty_list(self, tmp_path):
        """save_speaker_data with empty speaker_data returns empty list."""
        structure = {"speaker_data_dir": str(tmp_path / "speakers")}
        Path(structure["speaker_data_dir"]).mkdir(parents=True)
        builder = OutputStructureBuilder("test")
        out = builder.save_speaker_data({}, structure)
        assert out == []

    def test_metadata_file_contains_version_and_created_at(self, tmp_path):
        """create_metadata_file writes version and created_at."""
        builder = OutputStructureBuilder("m")
        structure = builder.create_standard_output_structure(
            str(tmp_path / "t.json"), base_output_dir=str(tmp_path)
        )
        path = builder.create_metadata_file(structure, {"k": "v"})
        data = json.loads(Path(path).read_text())
        assert data.get("version") == "1.0"
        assert "created_at" in data
        assert data.get("module") == "m"
