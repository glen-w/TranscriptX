...
class TestConfigBackwardCompat:
    """Backward compatibility: legacy config sections are ignored without error."""

    def test_config_ignores_unknown_transcription_section(self, tmp_path):
        """Config files with a 'transcription' section load successfully; section is silently ignored."""
        config_file = tmp_path / "config.json"
        config_data = {
            "transcription": {
                "model_name": "large-v2",
        ...
        assert config.analysis.sentiment_window_size == 10
        assert not hasattr(config, "transcription")
