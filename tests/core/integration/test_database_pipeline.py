...
class TestDatabaseIntegrationPipeline:
    ...
    def sample_analysis_results(self):
        ...
        return {
            ...
            "emotion": {
                "segments": [{"speaker": "SPEAKER_00", "dominant_emotion": "joy"}]
            },
        }
    ...
    def test_process_analysis_results_basic(self, pipeline, sample_analysis_results):
        ...
        with (
            patch.object(pipeline, "_validate_input_data"),
            ...
            result = pipeline.process_analysis_results(
                sample_analysis_results, conversation_id=1, speaker_ids=[1, 2]
            )
    ...
    def test_process_analysis_results_validation_error(
        self, pipeline, sample_analysis_results
    ):
        ...
        with patch.object(pipeline, "_validate_input_data") as mock_validate:
            ...
            with pytest.raises(ValueError):
                pipeline.process_analysis_results(
                    sample_analysis_results, conversation_id=1, speaker_ids=[1]
                )
    ...
    def test_process_speaker_data(self, pipeline, sample_analysis_results):
        ...
        with (
            patch.object(
                pipeline.extractors["sentiment"], "extract_speaker_data"
            ) as mock_extract,
            patch.object(
                pipeline.persistence_service, "store_speaker_data"
            ) as mock_store,
        ):
            ...
            result = pipeline._process_speaker_data(
                sample_analysis_results, conversation_id=1, speaker_id=1
            )
