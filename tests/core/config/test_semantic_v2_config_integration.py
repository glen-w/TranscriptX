"""Integration tests for semantic_similarity config resolution and persistence."""
...
def test_project_override_v2_resolves(config_dir) -> None:
    save_project_config({"analysis": {"semantic_similarity": {"batch_size": 32}}})
    resolved = resolve_effective_config(run_dir=None)
    assert resolved.effective_config.analysis.semantic_similarity.batch_size == 32
    assert (
...
def test_run_override_v2_wins_over_project(config_dir, tmp_path) -> None:
    ...
    save_project_config({"analysis": {"semantic_similarity": {"batch_size": 32}}})
    save_run_override(
        run_dir, {"analysis": {"semantic_similarity": {"batch_size": 16}}}
    )
...
def test_v2_roundtrip_save_load(config_dir) -> None:
    ...
    config.analysis.semantic_similarity.self_similarity_threshold = 0.81
    save_project_config(config.to_dict())
    resolved = resolve_effective_config(run_dir=None)
    assert (
        resolved.effective_config.analysis.semantic_similarity.self_similarity_threshold
        == 0.81
    )
...
def test_env_target_path_in_pydantic_model() -> None:
    assert "model_name" in SemanticSimilaritySettingsModel.model_fields
