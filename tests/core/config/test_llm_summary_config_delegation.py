"""Runtime delegation tests for analysis.llm_summary."""
...
from transcriptx.core.utils.config.analysis import AnalysisConfig, LLMSummaryConfig
...
_FIELDS = tuple(LLMSummarySettingsModel.model_fields.keys())
...
def test_default_shape_matches_pre_delegation_snapshot() -> None:
    assert_subtree_shape_matches_pre_snapshot("llm_summary")


def test_asdict_parity_with_pydantic_model() -> None:
    assert asdict(LLMSummaryConfig()) == LLMSummarySettingsModel().model_dump()


@pytest.mark.parametrize("field", _FIELDS)
def test_three_path_default_access(field: str) -> None:
    expected = LLMSummarySettingsModel().model_dump()[field]
    assert_three_path_access("llm_summary", field, expected)
...
def test_file_override_partial_merge(tmp_path: Path) -> None:
    ...
    config_file.write_text(
        json.dumps({"analysis": {"llm_summary": {"effort": "low"}}}),
        encoding="utf-8",
    )
    load_config_file_into(cfg, str(config_file))
    assert cfg.analysis.llm_summary.effort == "low"


def test_setattr_accepts_value_pydantic_would_reject_at_validation_boundary() -> None:
    cfg = AnalysisConfig()
    setattr(cfg.llm_summary, "effort", "extreme")
    assert cfg.llm_summary.effort == "extreme"
    errors = validate_config({"analysis": {"llm_summary": {"effort": "extreme"}}})
...
def test_is_dataclass_compatible_for_file_overrides() -> None:
    assert_is_dataclass_subtree("llm_summary")
