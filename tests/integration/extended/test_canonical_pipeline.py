...
# Fixture lives under tests/fixtures, not tests/integration/fixtures
FIXTURE_PATH = (
    Path(__file__).resolve().parents[2] / "fixtures" / "vtt" / "golden" / "simple.json"
)
...
@pytest.mark.xfail(
    reason="pipeline cache_hits reporting may differ when reusing run; needs pipeline review",
    strict=False,
)
...
@pytest.mark.xfail(
    reason="pipeline error propagation when module raises; mock point may have changed",
    strict=False,
)
