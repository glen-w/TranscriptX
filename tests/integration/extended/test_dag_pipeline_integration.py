...
_PIPELINE_CONTEXT_PATCH = "transcriptx.core.pipeline.dag_pipeline_run.PipelineContext"
...
class TestDAGPipelineIntegration:
    ...
    def sample_transcript_file(self, tmp_path):
        """Create a sample canonical schema v1.0 transcript file."""
    ...
    def test_full_pipeline_execution_with_dag(
        ...
        with (
            patch(_PIPELINE_CONTEXT_PATCH) as mock_context_class,
    ...
    def test_scaling_sanity_large_segments_no_pathological_explosion(
        self, sample_transcript_file
    ):
        """Scaling sanity: completion + bounded structural outputs, no timing assertions."""
        pipeline = DAGPipeline()
        mock_function = MagicMock(return_value={"status": "success"})

        # Dependency chain gives deterministic execution order under larger inputs.
        for i in range(12):
            deps = [f"scale_module_{i - 1}"] if i > 0 else []
            pipeline.add_module(
                f"scale_module_{i}",
                f"Scale Module {i}",
                "light",
        ...
        with (
            ...
            # Simulate larger transcript and many speakers without introducing runtime flakiness.
            segments = [
                {
                    "speaker": f"SPEAKER_{i % 30:02d}",
                    "text": f"Segment {i}",
                    "start": float(i),
                    "end": float(i) + 0.5,
                }
            ...
            speaker_map = {f"SPEAKER_{i:02d}": f"Speaker {i}" for i in range(30)}
            mock_context = MagicMock()
            mock_context.get_segments.return_value = segments
            mock_context.get_speaker_map.return_value = speaker_map
            mock_context.get_base_name.return_value = "scale_test"
            mock_context.validate.return_value = True
            mock_context_class.return_value = mock_context

            result = pipeline.execute_pipeline(
                transcript_path=sample_transcript_file,
                selected_modules=["scale_module_11"],
            )

        assert len(result["execution_order"]) == 12
        assert len(result["modules_run"]) <= 12
        assert isinstance(result["errors"], list)
