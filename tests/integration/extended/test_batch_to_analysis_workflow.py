...
from transcriptx.cli.batch_wav_workflow import _run_batch_wav_workflow_impl
...
class TestBatchToAnalysisWorkflow:
    ...
    def test_complete_batch_processing_flow(
        ...
        with (
            patch(
                "transcriptx.cli.batch_wav_workflow.select_folder_interactive"
            ) as mock_select,
            patch(
            ...
            patch(
                "transcriptx.cli.batch_wav_workflow.batch_process_files"
            ) as mock_process,
                ...
                "successful": [
                    {
                        "file": str(f),
                        "status": "success",
                        "transcript_path": str(sample_transcript_files[i]),
                    }
    ...
    def test_resume_capability(self, sample_wav_files, tmp_path):
        ...
        with (
            ...
            patch("transcriptx.cli.batch_wav_workflow.filter_new_files") as mock_filter,
            patch(
                "transcriptx.cli.batch_wav_workflow.resume_batch_processing"
            ) as mock_resume,
            patch(
                "transcriptx.cli.batch_wav_workflow.questionary.select"
            ) as mock_select_q,
    ...
    def test_mixed_success_failure_scenarios(self, sample_wav_files, tmp_path):
        ...
        with (
            ...
            mock_process.return_value = {
                "total_files": 3,
                "successful": [{"file": str(sample_wav_files[0]), "status": "success"}],
                "failed": [
                    {
                        "file": str(sample_wav_files[1]),
                        "status": "failed",
    ...
    def test_batch_with_post_processing_analysis(
        ...
        with (
            ...
            patch(
                "transcriptx.cli.batch_wav_workflow.run_batch_analysis_pipeline"
            ) as mock_analysis,
        ):
    ...
    def test_state_updates_for_each_file(self, sample_wav_files, tmp_path):
        ...
        with (
            ...
            mock_process.return_value = {
                "total_files": 3,
                "successful": [
                    {"file": str(f), "status": "success"} for f in sample_wav_files
                ],
                "failed": [],
                "skipped": [],
            }
