...
class TestDAGPipelineIntegration:
    ...
    def test_full_pipeline_execution_with_dag(self, sample_transcript_file, sample_speaker_map):
        ...
        pipeline.add_module("dependent", "Dependent Module", "medium", ["base"], dependent_module)
        ...
        with patch('transcriptx.core.pipeline.pipeline_context.PipelineContext') as mock_context_class, \
            ...
            assert result["execution_order"].index("base") < result["execution_order"].index("dependent")
    ...
    def test_module_dependency_chains(self, sample_transcript_file, sample_speaker_map):
        ...
        with patch('transcriptx.core.pipeline.pipeline_context.PipelineContext') as mock_context_class, \
            ...
            assert result["execution_order"].index("level1") < result["execution_order"].index("level2")
            assert result["execution_order"].index("level2") < result["execution_order"].index("level3")
    
    def test_error_propagation_through_dependencies(self, sample_transcript_file, sample_speaker_map):
    ...
    def test_partial_execution_and_recovery(self, sample_transcript_file, sample_speaker_map):
    ...
    def test_performance_with_multiple_modules(self, sample_transcript_file, sample_speaker_map):
        ...
        for i in range(10):
            ...
            pipeline.add_module(f"module{i}", f"Module {i}", "light", deps, mock_function)
        ...
        with patch('transcriptx.core.pipeline.pipeline_context.PipelineContext') as mock_context_class, \
            ...
            for i in range(1, 10):
                assert result["execution_order"].index(f"module{i-1}") < result["execution_order"].index(f"module{i}")
    
    def test_resource_usage_monitoring(self, sample_transcript_file, sample_speaker_map):
