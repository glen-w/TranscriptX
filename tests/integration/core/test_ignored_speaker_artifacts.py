"""Integration tests for ignored speaker artifacts."""
...
def _write_sidecar_all_ignored(transcript_path: Path) -> None:
    """Sidecar is the source of truth for ignored_speakers (not inline transcript JSON)."""
    sidecar = transcript_path.with_name(f"{transcript_path.stem}.speaker_map.json")
    sidecar.write_text(
        json.dumps({"ignored_speakers": ["Alice", "Bob"]}),
        encoding="utf-8",
    )
...
def test_ignored_speaker_creates_no_speaker_artifacts(
    ...
    _write_sidecar_all_ignored(transcript_path)
    ...
    stats_json = output_dir / "ignored_stats.json"
