...
def compare_transcripts(
    ...
    if len(segments1) != len(segments2):
        differences.append(
            f"Segment count differs: {len(segments1)} vs {len(segments2)}"
        )

    if ignore_speaker_order:
        # Sort segments by speaker and text for comparison
        segments1 = sorted(
            segments1, key=lambda s: (s.get("speaker", ""), s.get("text", ""))
        )
        segments2 = sorted(
            segments2, key=lambda s: (s.get("speaker", ""), s.get("text", ""))
        )
    ...
    for i in range(min_len):
        ...
        if seg1.get("speaker") != seg2.get("speaker"):
            differences.append(
                f"Segment {i}: speaker differs ({seg1.get('speaker')} vs {seg2.get('speaker')})"
            )
...
def compare_speaker_maps(map1: Dict[str, str], map2: Dict[str, str]) -> Dict[str, Any]:
    """
...
def compare_analysis_results(
    result1: Dict[str, Any], result2: Dict[str, Any], tolerance: float = 0.001
) -> Dict[str, Any]:
    """
    ...
    if result1.get("status") != result2.get("status"):
        differences.append(
            f"Status differs: {result1.get('status')} vs {result2.get('status')}"
        )
