...
def assert_valid_transcript(transcript: Dict[str, Any], strict: bool = True) -> None:
    ...
    if strict:
        for i, segment in enumerate(transcript["segments"]):
            ...
            if "start" in segment:
                assert isinstance(segment["start"], (int, float)), f"Segment {i} start must be numeric"
            if "end" in segment:
                assert isinstance(segment["end"], (int, float)), f"Segment {i} end must be numeric"
                if "start" in segment:
                    assert segment["end"] >= segment["start"], f"Segment {i} end must be >= start"
...
def assert_valid_speaker_map(speaker_map: Dict[str, str]) -> None:
    ...
    for speaker_id, name in speaker_map.items():
        assert isinstance(speaker_id, str), f"Speaker ID must be string, got {type(speaker_id)}"
...
def assert_valid_output_structure(
    ...
    if hasattr(output_structure, "__dict__"):
        ...
        raise AssertionError(f"Output structure must be dict or have __dict__, got {type(output_structure)}")
    
    for key in required_keys:
        assert key in structure_dict or hasattr(output_structure, key), \
...
def assert_json_valid(file_path: str, schema: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
