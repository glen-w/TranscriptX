"""
Tests for utils.py delegation to services.
"""
...
class TestWebUtilsDelegation:
    """Tests that utils.py delegates to services."""
    ...
    def test_load_transcript_by_session_delegates(self):
        """Test that load_transcript_by_session delegates to FileService."""
        with patch("transcriptx.web.utils.FileService") as mock_service:
            mock_service.load_transcript_by_session.return_value = {"segments": []}

            result = load_transcript_by_session("test_session")

            mock_service.load_transcript_by_session.assert_called_once_with(
                "test_session"
            )
