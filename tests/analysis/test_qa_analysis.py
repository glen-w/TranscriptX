...
class TestQAAnalysis:
    ...
    def sample_segments_with_questions(self):
        """Fixture for sample transcript segments with questions using segment-based speaker identification."""
        return [
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
    ...
    def sample_segments_pattern_questions(self):
        """Fixture for segments with questions detected by pattern (no acts data) using segment-based speaker identification."""
        return [
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
    ...
    def sample_segments_unanswered_questions(self):
        """Fixture for segments with unanswered questions using segment-based speaker identification."""
        return [
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
    ...
    def sample_acts_data(self):
        """Fixture for sample acts data using segment-based speaker identification."""
        return {
            "segments_with_acts": [
                {
                    "speaker": "Alice",
    ...
    def test_qa_analysis_basic(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
        """Test basic Q&A analysis."""
        result = qa_module.analyze(sample_segments_with_questions)
    ...
    def test_qa_analysis_question_detection(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_answer_matching(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_response_quality(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_unanswered_questions(
        self, qa_module, sample_segments_unanswered_questions, sample_speaker_map
    ):
        """Test detection of unanswered questions."""
        result = qa_module.analyze(sample_segments_unanswered_questions)

        unanswered = result["unanswered_questions"]
        # Should detect at least some unanswered questions
        assert isinstance(unanswered, list)

    def test_qa_analysis_question_classification(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_question_word_extraction(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_statistics(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_pattern_based_detection(
        self, qa_module, sample_segments_pattern_questions, sample_speaker_map
    ):
        """Test question detection using pattern matching (without acts data)."""
        result = qa_module.analyze(sample_segments_pattern_questions)
    ...
    def test_qa_analysis_with_acts_data(
        self,
        qa_module,
    ...
    def test_qa_analysis_question_chains(
        self, qa_module, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_empty_segments(self, qa_module):
        """Test Q&A analysis with empty segments."""
        segments = []

        result = qa_module.analyze(segments)

        assert "qa_pairs" in result
        assert "statistics" in result
        assert result["statistics"]["total_questions"] == 0

    def test_qa_analysis_no_questions(self, qa_module):
    ...
    def test_qa_analysis_single_speaker(self, qa_module):
        ...
        segments = [
            ...
            },
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
                "text": "I should check the documentation.",
                "start": 2.5,
                "end": 5.0,
            },
        ]
    ...
    def test_qa_analysis_custom_response_threshold(
        self, sample_segments_with_questions, sample_speaker_map
    ):
        """Test Q&A analysis with custom response time threshold."""
        module = QAAnalysis(config={"response_time_threshold": 5.0})

        result = module.analyze(sample_segments_with_questions)

        assert "qa_pairs" in result
        # Should use custom threshold for matching

    def test_qa_analysis_quality_weights(
        self, sample_segments_with_questions, sample_speaker_map
    ):
    ...
    def test_qa_analysis_run_from_context_no_acts(
        self, qa_module, pipeline_context_factory
    ):
    ...
    def test_qa_analysis_completeness_calculation(self, qa_module):
        ...
        completeness = qa_module._calculate_completeness(
            question_text, answer_text, "open_ended"
        )
