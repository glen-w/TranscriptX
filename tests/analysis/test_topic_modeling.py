...
class TestTopicModelingAnalysis:
    ...
    def test_topic_modeling_basic(
        ...
        mock_vec_instance.fit_transform.return_value = np.array(
            [[1, 2, 3], [2, 3, 4], [3, 4, 5]]
        )
        mock_vec_instance.get_feature_names_out.return_value = [
            "word1",
            "word2",
            "word3",
        ]
        ...
        mock_lda_instance.fit_transform.return_value = np.array(
    ...
    def test_topic_modeling_empty_segments(self, topic_module):
    ...
    @patch("transcriptx.core.analysis.topic_modeling.CountVectorizer")
    def test_topic_modeling_single_segment(
        self, mock_vectorizer, topic_module, sample_speaker_map
    ):
    ...
    @patch("transcriptx.core.analysis.topic_modeling.LatentDirichletAllocation")
    @patch("transcriptx.core.analysis.topic_modeling.CountVectorizer")
    def test_topic_modeling_speaker_aggregation(
        self,
        mock_vectorizer,
        mock_lda,
        topic_module,
        ...
        mock_lda_instance = MagicMock()
        mock_lda_instance.fit_transform.return_value = np.array(
            [[0.5, 0.5], [0.6, 0.4], [0.4, 0.6], [0.5, 0.5], [0.6, 0.4]]
        )
        mock_lda_instance.components_ = np.array([[0.3, 0.3, 0.4], [0.4, 0.3, 0.3]])
        mock_lda.return_value = mock_lda_instance
        ...
        assert (
            "speaker_topics" in result
            or "summary" in result
            or "speaker_topic_distributions" in result
        )

    @patch("transcriptx.core.analysis.topic_modeling.CountVectorizer")
    def test_topic_modeling_unicode_handling(
        self, mock_vectorizer, topic_module, sample_speaker_map
    ):
        ...
        segments = [
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
                "text": "This is a test with émojis 🎉 and unicode.",
                "start": 0.0,
                "end": 2.0,
            },
            {
                "speaker": "Bob",
                "speaker_db_id": 2,
                "text": "Another segment with special chars: ñ, ü, ç",
                "start": 2.0,
                "end": 4.0,
            },
        ]
