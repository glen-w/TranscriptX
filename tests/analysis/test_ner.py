...
class TestNERAnalysisModule:
    ...
    def sample_segments(self):
        """Fixture for sample transcript segments with entities and segment-based speaker identification."""
        return [
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
    ...
    def test_ner_analysis_basic(
        self, mock_spacy_load, ner_module, sample_segments, sample_speaker_map
    ):
        ...
        result = ner_module.analyze(sample_segments)

        assert "entities" in result or "segments" in result

    @patch("transcriptx.core.analysis.ner.spacy.load")
    def test_ner_analysis_person_entities(
        self, mock_spacy_load, ner_module, sample_speaker_map
    ):
        ...
        result = ner_module.analyze(segments)

        # Should extract person entities
        assert "entities" in result or "segments" in result

    @patch("transcriptx.core.analysis.ner.spacy.load")
    def test_ner_analysis_location_entities(
        self, mock_spacy_load, ner_module, sample_speaker_map
    ):
    ...
    @patch("transcriptx.core.analysis.ner.spacy.load")
    def test_ner_analysis_organization_entities(
        self, mock_spacy_load, ner_module, sample_speaker_map
    ):
    ...
    def test_ner_analysis_empty_segments(self, ner_module):
        ...
        assert "entities" in result or "segments" in result
        assert (
            len(result.get("entities", [])) == 0 or len(result.get("segments", [])) == 0
        )
