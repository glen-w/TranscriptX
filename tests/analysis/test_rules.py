...
rules = pytest.importorskip("transcriptx.core.analysis.rules")
CUE_PHRASES = rules.CUE_PHRASES
...
class TestRulesPatterns:
    ...
    def test_question_patterns(self):
        ...
        matches = any(re.search(pattern[0] if isinstance(pattern, tuple) else pattern, test_text, re.IGNORECASE) 
