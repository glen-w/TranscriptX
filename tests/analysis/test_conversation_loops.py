...
class TestConversationLoopDetector:
    ...
    def sample_segments(self) -> list[dict[str, Any]]:
        """Fixture for sample transcript segments with conversation loops using segment-based speaker identification."""
        return [
            {
                "speaker": "Alice",
                "speaker_db_id": 1,
    ...
    def test_detect_loops_basic(
        ...
        loops = detector.detect_loops(sample_segments)
    ...
    def test_detect_loops_no_loops(
        ...
        loops = detector.detect_loops(segments)
