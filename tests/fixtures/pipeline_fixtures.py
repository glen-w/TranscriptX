...
def create_mock_pipeline_context(
    ...
    Note: speaker_map parameter is deprecated. Segments may include
    ``speaker_db_id`` for stable per-speaker grouping.
    ...
    mock_context.speaker_map = (
        speaker_map  # Deprecated but kept for backward compatibility
    )
...
def create_mock_dag_pipeline():
    ...
    mock_pipeline.execute_pipeline = MagicMock(
        return_value={
