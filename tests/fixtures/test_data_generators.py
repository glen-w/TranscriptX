...
def generate_transcript(
    ...
    """
    segments = []
    # Use segment-based speaker identification
    ...
    for i in range(num_segments):
        ...
        if len(text) < min_text_length:
            text += " " + " ".join(
                random.choices(string.ascii_lowercase, k=min_text_length - len(text))
            )
        ...
        if include_timestamps:
            ...
            current_time += duration + random.uniform(
                0.5, 2.0
        ...
        if include_words:
            ...
            word_duration = (
                (segment.get("end", 0.0) - word_start) / len(words) if words else 0.0
            )
...
def generate_large_transcript(
    num_segments: int = 1000, num_speakers: int = 10
) -> Dict[str, Any]:
    """
...
def generate_edge_case_transcript(case: str = "empty") -> Dict[str, Any]:
    ...
    if case == "empty":
        ...
        return {
            "segments": [
                {"speaker": "SPEAKER_00", "text": long_text, "start": 0.0, "end": 100.0}
            ]
        }
...
def generate_malformed_transcript(variant: str = "missing_segments") -> Dict[str, Any]:
    ...
    if variant == "missing_segments":
        ...
        return {"segments": [{"text": "Missing speaker field"}]}
