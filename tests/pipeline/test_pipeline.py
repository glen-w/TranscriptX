...
from transcriptx.core.pipeline.pipeline import run_analysis_pipeline, run_analysis_pipeline_from_file
...
class TestRunAnalysisPipeline:
    def test_run_analysis_pipeline_success(self, temp_transcript_file):
        with patch("transcriptx.core.pipeline.pipeline.create_dag_pipeline") as mock_create_dag, \
             patch("transcriptx.core.pipeline.pipeline.validate_transcript") as mock_validate, \
             patch("transcriptx.core.pipeline.pipeline.generate_comprehensive_output_summary") as mock_summary, \
             patch("transcriptx.core.pipeline.pipeline.display_output_summary_to_user") as mock_display, \
    ...
    def test_run_analysis_pipeline_invalid_transcript(self, tmp_path):
        ...
        with patch("transcriptx.core.pipeline.pipeline.validate_transcript") as mock_validate:
    ...
    def test_run_analysis_pipeline_large_transcript(self, temp_transcript_file):
        large_segments = [
            {"speaker": f"SPEAKER_{i%5:02d}", "text": f"Segment {i}", "start": float(i), "end": float(i + 1)}
            for i in range(1000)
        ]
    ...
    def test_run_analysis_pipeline_performance_estimation(self, temp_transcript_file):
        with patch("transcriptx.core.pipeline.pipeline.create_dag_pipeline") as mock_create_dag, \
             ...
             patch("transcriptx.core.pipeline.pipeline.display_output_summary_to_user") as mock_display, \
             patch("transcriptx.io.transcript_loader.load_canonical_transcript") as mock_load_canonical, \
             patch("transcriptx.core.pipeline.pipeline.PerformanceEstimator") as mock_estimator:
            ...
            mock_est_instance.estimate_pipeline_time.return_value = {"estimated_seconds": 120}
    ...
    def test_run_analysis_pipeline_includes_execution_metadata(self, temp_transcript_file):
        ...
        with patch("transcriptx.core.pipeline.pipeline.create_dag_pipeline", return_value=mock_pipeline), \
