...
class TestResolveAnalysisTargetFilePath:
    ...
    def test_resolve_path_returns_scope_and_file_member(self, tmp_path: Path) -> None:
        ...
        scope, members = resolve_analysis_target(
            TranscriptRef(path=str(transcript_file))
        )
    ...
    def test_resolve_path_normalizes_and_uses_stem_for_display(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_resolve_path_never_touches_db(
        ...
        def raise_if_get_by_path(*args: object, **kwargs: object) -> None:
            ...
            raise RuntimeError(
                "get_transcript_file_by_path must not be called for path targets"
            )

        with (
            patch(
                "transcriptx.core.pipeline.target_resolver._get_session",
        ...
        assert (
            get_by_path_raised == []
        ), "path target must not call get_transcript_file_by_path"
        assert len(members) == 1 and members[0].file_path == str(
