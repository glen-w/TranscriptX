...
def _patch_minimal_single_run_dependencies(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        pipeline_module.CanonicalTranscript,
        "from_segments",
        lambda _segments: SimpleNamespace(
            content_hash="content-hash", schema_version="1.0"
        ),
    )
    monkeypatch.setattr(
        pipeline_module,
    ...
    monkeypatch.setattr(pipeline_module, "compute_file_hash", lambda _path: "file-hash")
    monkeypatch.setattr(
        "transcriptx.core.utils.slug_manager.register_transcript",
        lambda **_kwargs: "slug",
    )
    monkeypatch.setattr(
        pipeline_module, "set_transcript_output_dir", lambda *_args: None
    )
    monkeypatch.setattr(
        pipeline_module, "clear_transcript_output_dir", lambda *_args: None
    )
    monkeypatch.setattr(
