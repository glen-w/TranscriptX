...
from transcriptx.core.pipeline import module_registry as core_module_registry
from transcriptx.web import module_registry as web_module_registry
from transcriptx.app.workflows import analysis as workflow_analysis
...
class TestModuleRegistry:
    ...
    def test_shared_registry_accessor_returns_singleton(self):
        first = get_module_registry()
        second = get_module_registry()
        assert first is second

    def test_runtime_entrypoints_share_same_registry_object(self):
        shared = get_module_registry()
        assert core_module_registry.get_module_registry() is shared
        assert (
            workflow_analysis.get_available_modules
            is core_module_registry.get_available_modules
        )
        assert (
            web_module_registry.get_core_modules
            is core_module_registry.get_available_modules
        )
