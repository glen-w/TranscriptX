...
class TestDAGPipeline:
    ...
    def test_resolve_dependencies_with_deps(self):
        ...
        pipeline.add_module(
            "dependent", "Dependent Module", "medium", ["base"], mock_function
        )
    ...
    def test_resolve_dependencies_multiple_dependents(self):
        ...
        pipeline.add_module("base", "Base", "light", [], mock_function)
        pipeline.add_module(
            "dependent1", "Dependent1", "medium", ["base"], mock_function
        )
        pipeline.add_module(
            "dependent2", "Dependent2", "medium", ["base"], mock_function
        )
    ...
    def test_sort_by_category(self):
        ...
        execution_order = pipeline.resolve_dependencies(
            ["heavy1", "light1", "medium1", "light2"]
        )
    ...
    def test_execute_pipeline_success(
        self, mock_context_class, temp_transcript_file, sample_speaker_map
    ):
    ...
    @patch("transcriptx.core.pipeline.dag_pipeline.PipelineContext")
    def test_execute_pipeline_module_error(
        self, mock_context_class, temp_transcript_file, sample_speaker_map
    ):
        ...
        assert (
            "test_module" in str(result["errors"][0]).lower()
            or "error" in str(result["errors"][0]).lower()
        )
    ...
    def test_execute_pipeline_skips_multi_speaker_module(
        ...
        pipeline.add_module(
            "interactions", "Interactions", "medium", [], interactions_fn
        )
    ...
    def test_execute_pipeline_missing_dependency_error_message(
        ...
        with (
            patch(
                "transcriptx.core.pipeline.dag_pipeline.PipelineContext"
    ...
    def test_execute_pipeline_invalid_transcript(self, tmp_path):
        ...
        with patch(
            ...
            result = pipeline.execute_pipeline(
                transcript_path=str(invalid_file), selected_modules=["test_module"]
            )
...
class TestCreateDAGPipeline:
    ...
    def test_create_dag_pipeline(self):
        ...
        with patch(
            ...
            mock_registry_instance.get_available_modules.return_value = [
                "sentiment",
                "stats",
            ]
...
class TestDAGPipelineEdgeCases:
    ...
    def test_missing_dependency_handling(self):
        ...
        pipeline.add_module(
            "dependent", "Dependent", "medium", ["nonexistent"], mock_function
        )

        # Implementation raises ValueError when a selected module has missing deps
        with pytest.raises(ValueError, match="Missing dependencies"):
            pipeline.resolve_dependencies(["dependent"])
    ...
    def test_timeout_per_module(self):
        ...
        pipeline.add_module(
            "test", "Test", "light", [], mock_function, timeout_seconds=300
        )
    ...
    def test_large_dependency_graph(self):
        ...
        for i in range(15):
            ...
            pipeline.add_module(
                module_name, f"Module {i}", "light", deps, mock_function
            )
        ...
        for i in range(1, 15):
            assert execution_order.index(f"module{i-1}") < execution_order.index(
                f"module{i}"
            )
    ...
    def test_category_ordering_enforcement(self):
        ...
        execution_order = pipeline.resolve_dependencies(
            ["heavy1", "light1", "medium1", "light2", "medium2"]
        )
    ...
    def test_preflight_check_missing_module(self):
        ...
        assert "nonexistent" in results["skipped_modules"] or "nonexistent" in str(
            results["warnings"]
        )
...
class TestExecutePipelineFakeDAG:
    """
    Tests using a tiny fake DAG and lightweight fake coordinators.
    Do not mock the new helpers; assert on results and call logs.
    """
    ...
    def test_success_path_records_in_results_and_run_report(
        self, mock_context_class, _v1, _v2, temp_transcript_file, sample_speaker_map
    ):
        """Success path: results dict and run_report receive the completed module."""
        pipeline = DAGPipeline()
        fn_a = MagicMock(return_value={"status": "success", "artifacts": []})
        pipeline.add_module("mod_a", "Module A", "light", [], fn_a)

        run_report_calls = []

        class FakeRunReport:
            def record_module(self, **kwargs):
                run_report_calls.append(("record_module", kwargs))
        ...
        result = pipeline.execute_pipeline(
            transcript_path=str(temp_transcript_file),
            selected_modules=["mod_a"],
            speaker_map=sample_speaker_map,
            skip_speaker_mapping=True,
            run_report=FakeRunReport(),
        )

        assert result["modules_run"] == ["mod_a"]
        assert result["errors"] == []
        assert "mod_a" in result["module_results"]
        assert len(run_report_calls) >= 1
        record = run_report_calls[0]
        assert record[0] == "record_module"
        assert record[1].get("module_name") == "mod_a"
        assert record[1].get("status") == ModuleResult.RUN
    ...
    def test_exception_path_records_error_and_run_report_fail(
        ...
        """Exception path: error in results and run_report.record_module(FAIL) called."""
        pipeline = DAGPipeline()
        fn_a = MagicMock(side_effect=RuntimeError("simulated failure"))
        pipeline.add_module("mod_a", "Module A", "light", [], fn_a)
        ...
        assert "mod_a" in result["module_results"]
        assert result["module_results"]["mod_a"].get("status") == "error"
        fail_calls = [c for c in run_report_calls if c[1].get("error")]
        assert len(fail_calls) >= 1
        assert fail_calls[0][1].get("module_name") == "mod_a"
    ...
    def test_abort_logic_stops_loop_on_critical_error(
        ...
        """When a module fails with speaker-map critical error, pipeline stops (second module not run)."""
        pipeline = DAGPipeline()
        fn_first = MagicMock(
            side_effect=ValueError("speaker mapping required for this module")
        )
        fn_second = MagicMock(return_value={"status": "success"})
        pipeline.add_module("first", "First", "light", [], fn_first)
        pipeline.add_module("second", "Second", "light", [], fn_second)
        ...
        assert len(result["errors"]) > 0
        fn_second.assert_not_called()

    def test_should_abort_pipeline_returns_true_for_speaker_map_error(self):
        """_should_abort_pipeline returns True when error mentions speaker mapping."""
        pipeline = DAGPipeline()
        outcome = ModuleExecOutcome(
            status="failed",
            error="Speaker mapping required",
        )
        results = {}
        assert pipeline._should_abort_pipeline(outcome, results) is True
        assert results.get("status") == "failed"

    def test_should_abort_pipeline_returns_false_for_other_error(self):
        """_should_abort_pipeline returns False for non-critical errors."""
        pipeline = DAGPipeline()
        outcome = ModuleExecOutcome(
            status="failed",
        ...
        assert "status" not in results or results.get("status") != "failed"
    ...
    def test_event_collector_records_module_started_completed(
        ...
        """Structured event_collector receives module_started and module_completed."""
        pipeline = DAGPipeline()
        fn_a = MagicMock(return_value={"status": "success"})
        pipeline.add_module("mod_a", "Module A", "light", [], fn_a)
        ...
        started = [e for e in events if e.get("event") == "module_started"]
        completed = [e for e in events if e.get("event") == "module_completed"]
        assert len(started) == 1 and started[0]["module_name"] == "mod_a"
        assert len(completed) == 1 and completed[0]["module_name"] == "mod_a"
