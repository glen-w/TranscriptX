...
class TestPipelineContext:
    ...
    def test_pipeline_context_get_analysis_result_nonexistent(
    ...
    def test_pipeline_context_get_computed_value_nonexistent(
        self, pipeline_context_factory
    ):
    ...
    def test_pipeline_context_file_not_found(self, tmp_path):
        ...
        with patch(
            ...
            mock_service.load_transcript_data.side_effect = FileNotFoundError(
                "File not found"
            )
    ...
    def test_pipeline_context_set_segments(self, pipeline_context_factory):
        ...
        new_segments = [
            {"speaker": "SPEAKER_00", "text": "New segment", "start": 0.0, "end": 1.0}
        ]
    ...
    def test_pipeline_context_metadata_speaker_map(
        self, tmp_path, mock_transcript_service
    ):
        ...
        sidecar_path = transcript_path.with_name("test.speaker_map.json")
        sidecar_path.write_text(
            '{"speaker_map":{"SPEAKER_00":"Alice"},"ignored_speakers":[],"speaker_id_to_db_id":{},"speaker_map_schema_version":"1.0"}'
        )
        ...
        with patch(
            ...
            assert (
                context.get_speaker_display_name("SPEAKER_00") == "Alice (SPEAKER_00)"
            )

    def test_pipeline_context_get_speaker_display_name_normalizes_diarized_key(
        self, tmp_path, mock_transcript_service
    ):
        """Unpadded SPEAKER_3 query resolves when sidecar keys are normalized SPEAKER_03."""
        transcript_path = tmp_path / "meet.json"
        transcript_path.write_text(
            '{"segments":[{"speaker":"SPEAKER_3","text":"Hello"}]}'
        )
        sidecar_path = transcript_path.with_name("meet.speaker_map.json")
        sidecar_path.write_text(
            '{"speaker_map":{"SPEAKER_03":"Alice"},"ignored_speakers":[],"speaker_id_to_db_id":{}}'
        )
        ...
        with patch(
            ...
            assert context.get_speaker_display_name("SPEAKER_3") == "Alice (SPEAKER_3)"

    def test_pipeline_context_get_speaker_display_name_numeric_string_matches_sidecar(
        self, tmp_path, mock_transcript_service
    ):
        """Numeric diarization id '3' resolves when sidecar uses normalized SPEAKER_03."""
        transcript_path = tmp_path / "meet.json"
        transcript_path.write_text('{"segments":[{"speaker":"3","text":"Hello"}]}')
        sidecar_path = transcript_path.with_name("meet.speaker_map.json")
        sidecar_path.write_text(
            '{"speaker_map":{"SPEAKER_03":"Alice"},"ignored_speakers":[],"speaker_id_to_db_id":{}}'
        )
        ...
        with patch(
            ...
            assert context.get_speaker_display_name("3") == "Alice (3)"
    ...
    def test_pipeline_context_recovers_mapping_from_original_cue(
        self, tmp_path, mock_transcript_service
    ):
        """When sidecar is absent, mapping should be recovered from original_cue."""
        transcript_path = tmp_path / "test.json"
        transcript_path.write_text('{"segments": []}')
