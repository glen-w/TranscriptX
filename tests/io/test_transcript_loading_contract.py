...
class TestCanonicalTranscriptLoadingContract:
    ...
    def test_load_transcript_data_tuple_unpacking_still_works(
        ...
        """load_transcript_data (via loader) returns tuple that can be unpacked as segments, base_name, transcript_dir."""
        ...
        with patch("transcriptx.io.get_transcript_service") as mock_get:
            service = TranscriptService()
            service.load_transcript_data = lambda *a, **k: stub
            mock_get.return_value = service

            segments, base_name, transcript_dir = load_transcript_data(path)
...
class TestTranscriptServiceLoadTranscriptDataContract:
    ...
    def test_io_load_transcript_data_returns_named_tuple_with_segments(
        self, tmp_path: Path
    ) -> None:
        from transcriptx.io import load_transcript_data as io_load_td

        path, segments, _ = _fixture_transcript(tmp_path)
        result = io_load_td(path)
