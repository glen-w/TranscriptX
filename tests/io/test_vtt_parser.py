...
class TestTranscriptImporter:
    ...
    def test_import_raises_clear_error_when_sembly_html_requires_bs4(self, monkeypatch):
        """Surface missing parser dependency as a user-facing ValueError."""
        ...
        class _DummyAdapter:
            ...
            def parse(self, path, content):
                return IntermediateTranscript(
                    ...
                    source_metadata={},
                    warnings=[
                        "beautifulsoup4 is required to parse Sembly HTML exports"
                    ],
                )

        dummy_path = FIXTURES_DIR / "simple.vtt"

        monkeypatch.setattr(
            "transcriptx.io.transcript_importer.registry.detect",
            lambda *args, **kwargs: _DummyAdapter(),
        )
        monkeypatch.setattr(
            "transcriptx.io.import_core.pipeline.normalize_speakers",
            lambda turns: [],
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with pytest.raises(
                ValueError,
                match="Sembly HTML import requires 'beautifulsoup4'",
            ):
                import_transcript(dummy_path, output_dir=tmpdir)
