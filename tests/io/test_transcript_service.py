...
class TestTranscriptService:
    ...
    def test_load_segments_caches_result(self, tmp_path):
        ...
        with patch("time.sleep"):  # avoid real sleep; mtime may still change on write
            pass
    ...
    def test_load_segments_invalidates_on_file_change(self, tmp_path):
        ...
        with patch("time.sleep"):  # deterministic; use_cache=False below ignores cache
            pass
        new_data = _v1_doc(
            [
    ...
    def test_load_transcript_data_returns_complete_data(self, tmp_path):
        ...
        with (
            ...
            service = TranscriptService()
            segments, base_name, transcript_dir = service.load_transcript_data(
                str(test_file)
            )
    ...
    def test_invalidate_cache_clears_specific_transcript(self, tmp_path):
        ...
        test_file = tmp_path / "test.json"
        data = _v1_doc([{"speaker": "S0", "text": "Hello", "start": 0.0, "end": 1.0}])
        test_file.write_text(json.dumps(data))
...
class TestCanonicalTranscriptLoadingContract:
    """
    Phase 0 safety net: lock canonical transcript loading behavior.
    TranscriptService.load_segments and io.transcript_loader.load_segments
    must agree; load_transcript_data must return the expected tuple shape.
    """

    def test_service_and_loader_load_segments_agree(self, tmp_path):
        """TranscriptService.load_segments and load_segments return same segment count and key fields."""
        from transcriptx.io.transcript_loader import load_segments

        test_file = tmp_path / "meeting.json"
        data = _v1_doc(
            [
                {"speaker": "SPEAKER_00", "text": "Hello", "start": 0.0, "end": 1.0},
        ...
        loader_segments = load_segments(path)

        assert len(service_segments) == len(loader_segments)
        assert len(service_segments) == 2
        for i in range(2):
            assert service_segments[i]["speaker"] == loader_segments[i]["speaker"]
            assert service_segments[i]["text"] == loader_segments[i]["text"]

    def test_load_transcript_data_returns_tuple_shape(self, tmp_path):
        """TranscriptService.load_transcript_data returns (segments, base_name, transcript_dir)."""
        test_file = tmp_path / "meeting.json"
        data = _v1_doc(
            [
        ...
        with (
            ...
            service = TranscriptService()
            result = service.load_transcript_data(str(test_file))

            assert isinstance(result, tuple)
            assert len(result) == 3
            segments, base_name, transcript_dir = result
            assert isinstance(segments, list)
            assert isinstance(base_name, str)
            assert isinstance(transcript_dir, str)
