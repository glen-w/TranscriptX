...
class TestSemblyAdapterParseHTML:
    ...
    def test_parses_new_sembly_transcription_export_html(self):
        adapter = SemblyAdapter()
        html = """
        <html>
          <head>
            <script src="https://prodeusemblyaimain.blob.core.windows.net/example.js"></script>
          </head>
          <body>
            <h2>Supervision 9 may</h2>
            <p>• Mar 10, 2026, 10:54 AM - 11:54 AM • Audio Upload</p>
            <div><p><strong>Filka</strong> • 00:00</p><p>Right. Hi.</p></div>
            <div><p><strong>Ana</strong> • 00:18</p><p>It's on the...</p></div>
            <div><p><strong>Fede</strong> • 00:24</p><p>Before we start...</p></div>
          </body>
        </html>
        """.encode("utf-8")
        path = Path("sample_sembly_export.html")

        score = adapter.detect_confidence(path, html[:4096])
        assert score >= 0.8

        result = adapter.parse(path, html)
        assert len(result.turns) == 3
        assert [t.speaker for t in result.turns] == ["Filka", "Ana", "Fede"]
        assert result.turns[0].start == pytest.approx(0.0)
        assert result.turns[1].start == pytest.approx(18.0)
        assert result.turns[2].start == pytest.approx(24.0)
        assert result.turns[0].text == "Right. Hi."
