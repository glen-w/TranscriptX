...
WORD_LEVEL_FIXTURE = FIXTURES / "word_level.json"
...
class TestWhisperXAdapterParseWordLevel:
    ...
    def test_parity_with_word_level_fixture(self):
        """WhisperXAdapter should parse the pinned word-level fixture correctly."""
        adapter = WhisperXAdapter()
        path = WORD_LEVEL_FIXTURE
        content = path.read_bytes()
        result = adapter.parse(path, content)
...
class TestWhisperXSemanticParity:
    ...
    def test_word_level_speaker_promotion_import_path(self):
        """Adapter → normalizer → speaker_normalizer yields v1-ready segments (non-empty speaker)."""
        from transcriptx.io.transcript_normalizer import TranscriptNormalizer
        from transcriptx.io.speaker_normalizer import normalize_speakers
        ...
        for seg in new_segments:
            assert isinstance(seg["speaker"], str) and seg["speaker"].strip()
            assert seg["text"]
