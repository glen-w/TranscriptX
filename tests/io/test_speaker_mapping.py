...
- transcript JSON is updated via SpeakerMappingService (sole write authority)
...
from transcriptx.io.speaker_mapping.core import (
    _update_transcript_json_with_speaker_names,
)
...
def test_speaker_mapping_service_bulk_update(tmp_path) -> None:
    """Transcript JSON is updated via SpeakerMappingService.bulk_update (sole write authority)."""
    ...
    SpeakerMappingService().bulk_update(
        ...
        speaker_map,
        [],
        speaker_id_to_db_id=speaker_db_ids,
    )
...
def test_update_transcript_json_with_speaker_names_private(tmp_path) -> None:
    """Regression test for private helper _update_transcript_json_with_speaker_names (deprecated)."""
    import warnings
