...
class TestEndTimestampRepair:
    ...
    def test_repairs_end_when_end_less_than_start(self):
        t = IntermediateTurn(text="Hi", speaker=None, start=5.0, end=4.0, turn_index=0)
        transcript = _make_transcript(t)

        normalizer = TranscriptNormalizer(default_estimated_duration=2.0)
        turns = normalizer.normalize(transcript)

        assert turns[0].end == pytest.approx(7.0, abs=0.001)
        assert any("repaired" in w.lower() for w in transcript.warnings)

    def test_repairs_end_when_end_equals_start_using_next_start(self):
        t0 = IntermediateTurn(
            text="First", speaker=None, start=10.0, end=10.0, turn_index=0
        )
        t1 = IntermediateTurn(
            text="Second", speaker=None, start=12.5, end=13.0, turn_index=1
        )
        transcript = _make_transcript(t0, t1)
        ...
        assert turns[0].end == pytest.approx(12.5, abs=0.001)
