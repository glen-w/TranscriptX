...
from transcriptx.database.models import Speaker, TranscriptFile, SpeakerVocabularyWord
...
class TestVocabularyStorageService:
    ...
    def test_store_speaker_vocabulary_snapshot(self, service, sample_speaker, transcript_file):
        ...
        assert all(v.source_transcript_file_id == transcript_file.id for v in vocab_words)
        assert all(v.snapshot_version == 1 for v in vocab_words)
        assert all(v.tfidf_score > 0 for v in vocab_words)
    
    def test_vocabulary_snapshot_versioning(self, service, sample_speaker, transcript_file):
