...
from transcriptx.database.models import TranscriptSentence, SpeakerVocabularyWord, SpeakerResolutionEvent
...
class TestSentenceVocabularyIntegration:
    ...
    def segment_service(self, db_session):
        ...
        with pytest.MonkeyPatch().context() as m:
            from transcriptx.database import segment_storage, sentence_storage, vocabulary_storage, speaker_profiling
    ...
    def test_end_to_end_storage(self, segment_service, sample_transcript_data, db_session):
        ...
        db_segments = db_session.query(TranscriptSegment).filter_by(
            transcript_file_id=transcript_file.id
        ).all()
        segment_ids = [seg.id for seg in db_segments]
        
        sentences = db_session.query(TranscriptSentence).filter(
            TranscriptSentence.transcript_segment_id.in_(segment_ids)
        ).all()
        ...
        vocab_words = db_session.query(SpeakerVocabularyWord).filter(
            SpeakerVocabularyWord.speaker_id.in_(speakers)
        ).all()
    ...
    def test_sentence_speaker_assignment(self, segment_service, sample_transcript_data, db_session):
        ...
        # Get all sentences
        all_sentences = db_session.query(TranscriptSentence).join(
            TranscriptSentence.transcript_segment
        ).filter(
        ...
        for segment in db_segments:
            segment_sentences = [s for s in all_sentences if s.transcript_segment_id == segment.id]
            for sentence in segment_sentences:
                assert sentence.speaker_id == segment.speaker_id
    
    def test_vocabulary_snapshot_per_speaker(self, segment_service, sample_transcript_data, db_session):
        ...
        for speaker_id in speakers:
            vocab_words = db_session.query(SpeakerVocabularyWord).filter_by(
    ...
    def test_resolution_event_completeness(self, segment_service, sample_transcript_data, db_session):
    ...
    def test_analysis_run_id_propagation(self, segment_service, sample_transcript_data, db_session):
        ...
        sentences = db_session.query(TranscriptSentence).join(
            TranscriptSentence.transcript_segment
        ).filter(
            TranscriptSentence.transcript_segment.has(
                transcript_file_id=transcript_file.id
