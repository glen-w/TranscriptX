...
class TestSentenceStorageService:
    ...
    def test_store_sentences_from_segments(self, service, sample_segments):
        ...
        assert all(
            s.transcript_segment_id in [seg.id for seg in sample_segments]
            for s in sentences
        )
    ...
    def test_sentence_timestamp_distribution(self, service, sample_segments):
        ...
        first_segment_sentences = [
            s for s in sentences if s.transcript_segment_id == sample_segments[0].id
        ]
        ...
        for i in range(len(first_segment_sentences) - 1):
            assert (
                first_segment_sentences[i].start_time
                <= first_segment_sentences[i + 1].start_time
            )
            assert (
                first_segment_sentences[i].end_time
                <= first_segment_sentences[i + 1].end_time
            )
    ...
    def test_empty_segment_handling(
        self, service, db_session, transcript_file, sample_speaker
    ):
    ...
    def test_analysis_run_id(self, service, sample_segments):
        ...
        sentences = service.store_sentences_from_segments(
            sample_segments, analysis_run_id=run_id
        )
