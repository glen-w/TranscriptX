...
class TestTopicModelingContracts:
    ...
    def test_topic_modeling_insufficient_segments_output_contract(
        ...
        result = topic_module.analyze(segments)
    ...
    def test_topic_modeling_success_output_contract(
        ...
        result = topic_module.analyze(sample_segments)
        ...
        assert "rejected_topics" in result
        assert set(result["rejected_topics"].keys()) == {"lda", "nmf"}

    @patch(
        "transcriptx.core.analysis.topic_modeling.analysis.perform_enhanced_lda_analysis"
    )
    def test_topic_modeling_dependency_error_contract(
        self,
        mock_lda: MagicMock,
        topic_module: TopicModelingAnalysis,
        sample_segments: list[dict[str, Any]],
        ...
        """Missing dependency paths should produce stable error envelope keys."""
        mock_lda.side_effect = ImportError("scikit-learn unavailable")
        context = MagicMock()
        context.transcript_path = "/tmp/input.json"
        context.get_segments.return_value = sample_segments
        context.get_speaker_map.return_value = sample_speaker_map
        context.get_base_name.return_value = "input"
        context.get_transcript_dir.return_value = "/tmp/out"
        context.get_run_id.return_value = "run-1"
        context.get_runtime_flags.return_value = {}
        context.get_analysis_result.return_value = None

        result = topic_module.run_from_context(context)

        assert result["module_name"] == "topic_modeling"
