...
# Plugins (fixture modules) used by database, regression, utils, and other tests; must be top-level.
pytest_plugins = [
...
def pipeline_context_factory(mock_transcript_service, temp_transcript_file):
    ...
    def _create_context(
        ...
        default_speaker_map = (
            speaker_map or {}
        )  # Empty by default - using database-driven approach
...
# Real DB fixtures (lazy imports to avoid circular import with transcriptx.database
# when conftest loads). Used by tests/cli/test_analysis_target_picker.py and
# tests/database/*.


@pytest.fixture(scope="session")
def test_database_url() -> str:
    """Temporary SQLite URL for test database."""
    import tempfile
...
def test_database_engine(test_database_url):
    """Session-scoped SQLite engine; creates/drops tables."""
    import shutil
...
def db_session(test_database_engine):
    """Function-scoped DB session with rollback after each test."""
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "/tests/integration/core/" in path_str:
            ...
            if not any(
                marker.name == "integration_core" for marker in item.iter_markers()
            ):
