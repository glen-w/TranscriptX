...
- **No recursive delete at root:** No `rmtree`/`rm -rf` on `.`, `..`, or any path that could contain the whole repo (e.g. workspace root, `src/`, or any dir containing `pyproject.toml`). If a command would delete outside `data/outputs/` or another known-safe subdir, do not run the apply step and report the risk.
- **Flag or abort on large deletions:** Before deleting many files under `data/outputs/` (e.g. old `test__*` run dirs):
  - List or estimate size/count first.
  - If the scope suggests a **huge** deletion (e.g. > ~1 GB or very large file count outside a small `test__*` area), **do not delete**; report and skip.
...
- **Test output cleanup:** There is no `transcriptx … clean-test-artifacts` subcommand. If the user asks to clean test artifacts, list candidate dirs under `data/outputs/` (e.g. `test__*`) and only delete with explicit confirmation; do not run bulk deletes without preview.
...
- **Run formatter**
  Use the project formatter: `black src/ tests/ scripts/*.py` (config in `pyproject.toml`). Do not run `black .` at repo root (can touch .venv/site-packages).
- **Sort imports**
