...
| `scripts/audio_preprocess.py` | Assess/preprocess audio before external transcription | power user | transcription.md; ROADMAP theme G1 | yes | yes | writes audio | any | pydub/ffmpeg | valid | maintainer | retain helper; **consider remove under G1** (not core) |
| `scripts/audio_merge.py` | Merge split recordings into one MP3 | power user | transcription.md; ROADMAP theme G1 | yes | yes | writes audio | any | ffmpeg | valid | maintainer | retain helper; **G1** invest (inline audio±transcript merge) vs remove |
