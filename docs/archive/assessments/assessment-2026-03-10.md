...
> **Superseded** by [`docs/dev/stocktake_2026-07-17.md`](../dev/stocktake_2026-07-17.md) (2026-07-17).  
> Status: Historical assessment only — kept for context. Metrics and architecture claims below are **not** current. See also `docs/dev/COMPLEXITY_GATES.md` and `docs/ROADMAP.md`.
...
| 6. Operational & Runtime Readiness | Good | CLI healthy; Docker hygiene good; schema migration tooling unconfigured at root |
...
- `core_mode = True` by default — currently this does not exclude modules; it primarily controls install/download behavior.
...
### Migration chain
- The `migrations/` was previously used for programmatic schema changes; the release process should now treat migrations as removed
- There is no active database migration chain in this repository
- This is a file-native project, so readiness checks focus on file manifests and JSON sidecars instead of database tooling
...
- **Dockerfile quality: Good**
  ...
  - `ENTRYPOINT ["transcriptx"]` launches the web UI; default `CMD` is `--host 0.0.0.0` ✓
- **docker-compose.yml quality: Good (current layout)**
  - Primary service: `transcriptx-web` (Streamlit on port 8501) ✓
  - Health check on the web service ✓
  ...
  - *Historical note:* older setups used separate `transcriptx`, `transcriptx-web`, and Speaker Studio services; those extra services are no longer in the default compose file.

### .env.example
- Placeholder values only (no secrets) ✓
- Documents `HOST_*` compose mount sources and `TRANSCRIPTX_*` keys used by paths, bootstrap, `env_overrides`, NLP/runtime, and web entrypoint ✓
...
| P2-6 | historical migration-tool references in docs should be removed to reflect file-native architecture | docs |
