...
### Phase 2 — Pluggable transcription providers (non-goal for v0.1)

TranscriptX is analysis-only; transcription is external (see [DECISIONS.md](DECISIONS.md) and [transcription.md](transcription.md)). A future phase could introduce a **TranscriptionProvider** protocol (e.g. `transcribe(audio_path) -> TranscriptDocument`) with optional built-in providers (WhisperX via HTTP, AssemblyAI, Deepgram, Colab). Configuration would be e.g. `transcriptx.provider.transcription = "whisperx-local"`. No provider code ships in v0.1; users bring their own transcript JSON.
