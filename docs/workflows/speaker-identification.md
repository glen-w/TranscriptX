...
3. Assign a clear display name with **Save name**, or **Ignore** a junk label.
4. Switch speakers with **Next** / **Prev** or **Jump to speaker**, then continue naming.
...
4. Enter a display name in **Assign name** and choose **Save name**. The page advances to the next unnamed speaker when one remains.
...
5. Use **Next** and **Prev** (or **Jump to speaker**) to move between diarized IDs without saving. Sample lines and clip targets follow the active speaker so you can compare voices before committing a name.
...
7. If a diarized ID is clearly noise (rare in this short fixture), choose **Ignore** so it does not pollute speaker summaries. You can open that speaker again and choose **Unignore** if you change your mind.
...
> `TX_SPEAKER_ID_WORKSPACE_COMPONENT=0`. Naming, ignore, and navigation controls
> for this walkthrough remain the same either way. See
...
- **Ignore** is for unusable diarization IDs; prefer naming real participants. **Unignore** restores a previously ignored ID.
