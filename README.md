...
**Golden path:** 1. **Get JSON** (e.g. from [WhisperX or another tool](docs/transcription.md)) → 2. **Canonicalize/validate** (`transcriptx transcript canonicalize --in <file>`, `transcriptx transcript validate --file <file>`) → 3. **Analyze** (`transcriptx analyze --transcript-file <file>`). See **[docs/transcription.md](docs/transcription.md)** for the full external-transcription guide.
...
# Interactive CLI (full terminal: use run so you get arrow keys and host-like TTY)
...
- External transcription workflow (bring your own WhisperX or other tool; see [docs/transcription.md](docs/transcription.md))
