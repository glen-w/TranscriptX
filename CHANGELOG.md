...
### Added
- **Docker-native contract**: Container I/O contract documented in `docs/docker.md`; output contract spec at `docs/contracts/output-contract-v1.md`.
- **RunManifestInput**: Canonical pipeline entry; `transcriptx analyze --manifest <path>` and `transcriptx batch-analyze --manifest <path>` run from a JSON run input. CLI flags normalize to `RunManifestInput` before the pipeline.
- **Speaker Studio MVP**: `SpeakerStudioController` orchestrator; Streamlit page uses only the controller. `docker compose up transcriptx-studio` → http://localhost:8502 (gated by `TRANSCRIPTX_ENABLE_SPEAKER_STUDIO=1`).
- **TranscriptStore**: Single write authority for transcript JSON (lockfile + atomic write). All mapping and canonicalize/import writes go through it.
- **Manifest type discriminators**: `manifest.json` has `manifest_type: "artifact_manifest"`; `.transcriptx/manifest.json` has `manifest_type: "run_manifest"`. Typed loaders `load_artifact_manifest()` and `load_run_manifest()`.

### Changed
- **Speaker mapping writes**: All transcript mapping writes go through `SpeakerMappingService` (which uses `TranscriptStore`). No raw `json.dump` to transcript files outside `TranscriptStore`.
- **Config**: Removed `transcription` from required config sections (analysis-only).
- **Output conventions**: Authority moved to `docs/contracts/output-contract-v1.md`; `docs/output_conventions.md` redirects there.

### Deprecated
- **`_update_transcript_json_with_speaker_names`**: Direct use deprecated; use `SpeakerMappingService` for all mapping writes. One-release grace period.
...
- **`TranscriptionConfig`** and env vars `TRANSCRIPTX_MODEL_NAME`, `TRANSCRIPTX_COMPUTE_TYPE`, `TRANSCRIPTX_LANGUAGE`, `HF_TOKEN`, `TRANSCRIPTX_HUGGINGFACE_TOKEN`.
- **`batch-process`**: Replaced by **`prep-audio`** (audio conversion/compression) and **`batch-analyze`** (bulk analysis on transcript JSONs).
- **WhisperX from default Docker Compose**: No `whisperx` service or Docker socket; image no longer installs `docker.io`. Standalone WhisperX reference files moved to `docs/recipes/whisperx/`.
...
- **Version**: Bumped to 0.41 across pyproject.toml, package __init__, web __init__, setup.py, ROADMAP, and this changelog.
...
- **v0.41** (Current): Speaker map migration, module consolidation, run manifest, current release baseline
