...
def upgrade() -> None:
    ...
    if _has_table("transcript_files"):
        with op.batch_alter_table("transcript_files") as batch:
            ...
            if not _has_column("transcript_files", "transcript_content_hash"):
                batch.add_column(
                    sa.Column("transcript_content_hash", sa.String(length=64))
                )
            ...
            if not _has_column("transcript_files", "sentence_schema_version"):
                batch.add_column(
                    sa.Column("sentence_schema_version", sa.String(length=50))
                )
    ...
    if not _has_table("transcript_speakers"):
        op.create_table(
            ...
            sa.Column(
                "created_at", sa.DateTime(), server_default=text("CURRENT_TIMESTAMP")
            ),
            sa.UniqueConstraint(
                "transcript_file_id",
                "speaker_label",
                name="uq_transcript_speaker_label",
            ),
        )
        op.create_index(
            "idx_transcript_speaker_file", "transcript_speakers", ["transcript_file_id"]
        )

    # Transcript segments/sentences: transcript_speaker_id
    if _has_table("transcript_segments") and not _has_column(
        "transcript_segments", "transcript_speaker_id"
    ):
        with op.batch_alter_table("transcript_segments") as batch:
            batch.add_column(
                sa.Column(
                    ...
                    sa.Integer(),
                    sa.ForeignKey("transcript_speakers.id", ondelete="SET NULL"),
                )
            )
    ...
    if _has_table("transcript_sentences") and not _has_column(
        "transcript_sentences", "transcript_speaker_id"
    ):
    ...
    if not _has_table("pipeline_runs"):
        op.create_table(
            ...
            sa.Column(
                "updated_at", sa.DateTime(), server_default=text("CURRENT_TIMESTAMP")
            ),
        )
        op.create_index(
            "idx_pipeline_run_file", "pipeline_runs", ["transcript_file_id"]
        )
    ...
    if not _has_table("module_runs"):
        op.create_table(
            ...
            sa.Column(
                "replaces_module_run_id",
                sa.Integer(),
                sa.ForeignKey("module_runs.id", ondelete="SET NULL"),
            ),
        ...
        op.create_index(
            "idx_module_run_input", "module_runs", ["module_name", "module_input_hash"]
        )
    ...
    if not _has_table("performance_spans"):
        op.create_table(
            ...
            sa.Column(
                "transcript_file_id",
                sa.Integer(),
                sa.ForeignKey("transcript_files.id", ondelete="SET NULL"),
            ),
            sa.Column(
                "created_at", sa.DateTime(), server_default=text("CURRENT_TIMESTAMP")
            ),
        ...
        op.create_index(
            "idx_span_start_time", "performance_spans", [sa.text("start_time DESC")]
        )
        op.create_index(
            "idx_span_name_start_time",
            "performance_spans",
            ["name", sa.text("start_time DESC")],
        )
        ...
        op.create_index(
            "idx_span_transcript", "performance_spans", ["transcript_file_id"]
        )
    ...
    if not _has_table("artifact_index"):
        ...
        op.create_index(
            "idx_artifact_transcript", "artifact_index", ["transcript_file_id"]
        )
...
def downgrade() -> None:
    ...
    if _has_table("transcript_segments") and _has_column(
        "transcript_segments", "transcript_speaker_id"
    ):
        with op.batch_alter_table("transcript_segments") as batch:
            batch.drop_column("transcript_speaker_id")

    if _has_table("transcript_sentences") and _has_column(
        "transcript_sentences", "transcript_speaker_id"
    ):
        with op.batch_alter_table("transcript_sentences") as batch:
            batch.drop_column("transcript_speaker_id")
