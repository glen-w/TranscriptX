...
def _run_analysis_workflow_impl(
    ...
    try:
        ...
        try:
            ...
            get_transcript_dir(str(transcript_file))
...
def _show_post_analysis_menu(transcript_file: Path, results: dict[str, Any]) -> None:
    ...
    txt_path = (
        ...
        / "data"
        / "global"
        / f"{analyzed_base}_comprehensive_summary.txt"
    )
    ...
    while True:
        ...
        try:
            ...
            if analysis_status:
                analysis_status.get("status", "unknown")
...
# (e.g. placeholder/test artifacts like default transcription output names)
...
def run_analysis_non_interactive(
    ...
    # Resolve output_dir for manifest (pipeline uses it when provided)
    ...
    try:
        ...
        manifest = RunManifestInput.from_cli_kwargs(
            ...
            include_unidentified_speakers=(
                speaker_options.include_unidentified if speaker_options else False
            ),
            skip_speaker_gate=skip_speaker_mapping,
            persist=persist,
        )
