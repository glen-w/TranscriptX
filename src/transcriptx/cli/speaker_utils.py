...
def check_speaker_identification_status(
    ...
    """
    transcript_data = load_transcript(str(transcript_path))
    speaker_map = (
        transcript_data.get("speaker_map", {})
        if isinstance(transcript_data.get("speaker_map"), dict)
        else {}
    )
    segments = load_segments(str(transcript_path), data=transcript_data)
...
def _speaker_gate_for_paths(
    ...
    *,
    ...
    """
    Shared speaker gate logic for batch and group flows.

    Callers must pass config explicitly so this helper is testable without
    monkeypatching get_config(). Path resolution and per-path exception
    handling (e.g. for group) remain in the callers.
    """
    ...
    if batch and config.mode == "ignore":
        total_segments = sum(
    ...
    if batch and config.exemplar_count > 0:
    ...
    choice = questionary.select(
        ...
        choices=[
            questionary.Choice(label, value=decision)
            for label, decision in _speaker_gate_choice_specs(config.mode, batch=batch)
        ],
        default=SpeakerGateDecision.IDENTIFY,
    ).ask()
...
def check_batch_speaker_gate(
    ...
    """
    config = get_config().workflow.speaker_gate
    return _speaker_gate_for_paths(
        transcript_paths,
...
def check_group_speaker_preflight(
    ...
    for path in resolved_paths:
        try:
            check_speaker_identification_status(path)
