"""
Interactive file picker UI (prompt_toolkit/questionary).

Use select_files_interactive() and FileSelectionConfig for interactive file selection
with optional playback, rename, and metadata. Utilities for discovering and listing
files live in file_selection_utils.
"""
...
def _apply_validator(files: List[Path], config: FileSelectionConfig) -> List[Path]:
    """Filter files by config.validator; return all if no validator."""
    if not config.validator:
        return list(files)
    return [f for f in files if config.validator(f)[0]]


def _build_initial_choices_and_cache(
    files: List[Path],
    all_files: List[Path],
    formatter: Callable[[Path], str],
) -> Tuple[List[Tuple[Path, str]], dict, dict]:
    """
    Build initial (path, label) choices and metadata cache for the file list.

    Returns:
        (choices, file_metadata_cache, original_labels). Choices do not include
        "Select All"; caller may insert it. Cache and labels are keyed by Path.
    """
    file_metadata_cache: dict = {}
    original_labels: dict = {}
    for file_path in all_files:
        try:
            label = formatter(file_path)
    ...
    return choices, file_metadata_cache, original_labels


def _build_shortcuts_for_help(
    config: FileSelectionConfig, hidden_files: List[Path]
) -> List[Tuple[str, str, str]]:
    """Build list of (icon, label, key) for help text."""
...
def _process_selection_result(
    result: Any,
    files: List[Path],
    config: FileSelectionConfig,
    select_all_value: Any,
) -> Optional[List[Path]]:
    """Process raw selection result into list of selected paths (with validator)."""
    if result is None:
        return None
...
def select_files_interactive(
    ...
    if (
        ...
        visible_files = _apply_validator(list(files), config)
    ...
    choices, file_metadata_cache, original_labels = _build_initial_choices_and_cache(
        files, all_files, initial_formatter
    )
    ...
    shortcuts = _build_shortcuts_for_help(config, hidden_files)
    ...
    try:
        ...
        return _process_selection_result(result, files, config, select_all_value)
