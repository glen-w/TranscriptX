...
def _process_one_file(
    ...
    *,
    ...
    if not dry_run and would_write:
        ...
        # each segment index the correct label before the service can replace with names.
        if strategy == STRATEGY_CASE2B and rewrite_segment_speakers:
            ...
            def patch_segments_mutator(data_patch: Dict[str, Any]) -> None:
            ...
            try:
                _export_store.mutate(
                    transcript_path,
                    patch_segments_mutator,
                    reason="export_speaker_maps_case2b",
                    timeout=15,
                )
