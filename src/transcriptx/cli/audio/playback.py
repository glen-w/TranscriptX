...
# Env: when set, segment playback also exports the clip here (for Docker / headless)
PLAYBACK_EXPORT_DIR_ENV = "TRANSCRIPTX_PLAYBACK_EXPORT_DIR"
...
def _extract_segment_ffmpeg(
    audio_path: Path, start: float, duration: float, output_path: Path
) -> bool:
    """Extract a segment to output_path using ffmpeg. Returns True on success."""
...
def export_segment_to_dir(
    ...
    *,
    ...
    """
    Export an audio segment to a WAV file in out_dir for playback in Docker/headless.
    Uses ClipService when available; falls back to ffmpeg. Returns the path of the written file, or None on failure.
    """
    if not audio_path.exists():
        return None
    start = max(0.0, start_s)
    duration = max(min_duration, float(end_s) - start)
    ...
    try:
        ...
        logger.warning(f"Playback export dir not writable: {out_dir}: {e}")
        return None
    safe_stem = "".join(
        c if c.isalnum() or c in "._-" else "_" for c in audio_path.stem
    )
    name = f"{safe_stem}_{start:.1f}-{start + duration:.1f}s.wav"
    output_path = out_dir / name
    try:
        from transcriptx.services.speaker_studio import ClipService

        clip_svc = ClipService()
        cached = clip_svc.get_clip_path(
            audio_path, start, start + duration, format="wav"
        )
        import shutil

        shutil.copy(cached, output_path)
        logger.debug(f"Exported segment to {output_path}")
        return output_path
    except Exception as e:
        logger.debug(f"ClipService export fallback: {e}")
    if _extract_segment_ffmpeg(audio_path, start, duration, output_path):
        logger.debug(f"Exported segment to {output_path}")
        return output_path
    return None
...
class SegmentPlayer:
    ...
    def last_export_path(self) -> Optional[Path]:
        """Path of the last segment exported to TRANSCRIPTX_PLAYBACK_EXPORT_DIR (Docker/headless)."""
    ...
    def play_segment(
        ...
        *,
        ...
        # When running in Docker/headless, export segment to a dir so user can play on host
        export_dir = os.environ.get(PLAYBACK_EXPORT_DIR_ENV)
        if not export_dir and Path("/.dockerenv").exists():
            export_dir = "/data/playback-clips"
        if export_dir:
            self._last_export_path = export_segment_to_dir(
                audio_path,
                start,
                start + duration,
                Path(export_dir),
