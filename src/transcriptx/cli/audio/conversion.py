...
def convert_audio_to_mp3(
    ...
    """
    Convert an audio file (WAV, MP3, OGG, etc.) to MP3 format.
    ...
    input_file_size_mb = audio_path.stat().st_size / (1024 * 1024)
    ...
    output_filename = audio_path.stem + ".mp3"
    output_path = output_dir / output_filename

    counter = 1
    while output_path.exists():
        output_filename = f"{audio_path.stem}_{counter}.mp3"
        output_path = output_dir / output_filename
        counter += 1

    from transcriptx.core.utils.performance_logger import TimedJob

    with TimedJob("audio.convert.audio_to_mp3", audio_path.name) as job:
        job.add_metadata(
        ...
        try:
            if progress_callback:
                progress_callback(0, 100, f"Loading {audio_path.name}...")
            ...
            if progress_callback:
                progress_callback(80, 100, "Converting to MP3...")
            ...
            logger.info(f"Converted {audio_path.name} to {output_path.name}")

            return output_path

        except CouldntDecodeError as e:
            error_msg = f"Could not decode audio file {audio_path.name}: {str(e)}"
            ...
            error_msg = f"Error converting {audio_path.name} to MP3: {str(e)}"
            log_error("AUDIO_CONVERSION", error_msg, exception=e)
            raise RuntimeError(error_msg)
...
def convert_wav_to_mp3(
    ...
    """
    Convert a WAV file to MP3 format. Delegates to convert_audio_to_mp3.
    """
...
def merge_audio_files(
    ...
    try:
        ...
        for idx, path in enumerate(audio_paths):
            if progress_callback:
                progress_msg = f"Loading {path.name} ({idx + 1}/{total_files})..."
        ...
        error_msg = f"Could not decode one of the audio files: {str(e)}"
...
def merge_wav_files(
    ...
    """
    Merge multiple WAV files into a single MP3 file. Delegates to merge_audio_files.
    """
