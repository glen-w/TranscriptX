...
def _render_section_b(audio_paths: List[Path]) -> None:
    ...
    if len(audio_paths) > 1:
        ...
        batch_suggested_steps: List[str] = []
        _render_section_c(audio_paths, batch_suggested_steps)
        return
...
def _render_result(result: object, audio_paths: List[Path]) -> None:
    ...
    if isinstance(result, list):
        ...
        for path, res in batch:
            ...
            if res.output_path:
                _render_transcription_handoff(res.output_path)
        return
    ...
    if single_result.output_path:
        _render_transcription_handoff(single_result.output_path)
