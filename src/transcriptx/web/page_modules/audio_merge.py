...
def _format_merge_row_meta(path: Path) -> str:
    """Format duration and size for a merge list row using fast metadata (no pydub)."""
    parts: List[str] = []
    try:
...
def _render_section_1(recordings: List[Path]) -> None:
    ...
    # Use fast metadata (ffprobe + stat) so we don't block on pydub loading full files
...
def _render_rename_section(merged_order: List[str], all_labels: dict[str, str]) -> None:
    ...
    st.caption(
        "Rename a selected recording together with its linked transcript so base names match."
    )
