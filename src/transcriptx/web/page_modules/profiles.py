...
from transcriptx.web.state import SELECTBOX_PLACEHOLDER_MODULE
    ...
    try:
        ...
        module_choice = st.selectbox(
            ...
            format_func=lambda m: (
                SELECTBOX_PLACEHOLDER_MODULE if m == "" else m.replace("_", " ").title()
            ),
            index=0,
            key="profiles_module",
        )
        if not module_choice:
            st.info("Select a module to view profiles.")
            return
