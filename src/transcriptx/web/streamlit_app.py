...
def render_transcript_viewer():
    ...
    try:
        ...
        selected = st.selectbox(
            "Select Session",
            [""] + session_names,
            format_func=lambda x: SELECTBOX_PLACEHOLDER_SESSION if x == "" else x,
            index=0,
            key="transcript_session_selector",
        )

        if not selected:
            st.info("Select a session to view the transcript.")
            return
...
def render_documentation():
    ...
    try:
        ...
        # Load module descriptions from markdown files (stub: fallback uses get_module_info)
        def _load_module_docs(module_name: str):
            return None

        module_descriptions = {}
        for module_name in modules:
            doc_info = _load_module_docs(module_name)
            if doc_info:
        ...
        legacy_descriptions = {
            ...
            "wordclouds": {
                ...
                "description": "Generates word clouds from transcript text using various approaches to visualize frequently used words. On **group runs**, word clouds are written via the wordclouds aggregation path (not `GROUP_CHART_REGISTRY`); Studio tags them `group_visual_special_path` so they are distinguishable from registry-backed aggregate charts.",
                "capabilities": [
                    "Generates word clouds",
                    "Visualizes frequently used words",
                    "Multiple generation approaches",
                    "Group runs: special visual path (see docs/groups/group_analysis_module_outputs.md)",
                ],
            },
            "tics": {
            ...
            "temporal_dynamics": {
                ...
                "description": "Analyzes how conversation patterns, topics, and dynamics change over time throughout the session. On **group runs**, this module produces **aggregated row data** (CSV/JSON) for the group; there is **no** group-level chart in `GROUP_CHART_REGISTRY` by design. Use single-transcript runs or reports for temporal dashboard charts.",
                "capabilities": [
                    "Tracks temporal changes in conversation",
                    "Analyzes dynamics over time",
                    "Pattern evolution analysis",
                    "Group runs: data outputs only (no registry-backed group charts)",
                ],
            },
            "qa_analysis": {
        ...
        with st.expander("Using Modules"):
            ...
            1. Process a transcript using the web app or Python API
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Navigation",
            ["Dashboard", "Transcript", "Documentation"],
            index=(
                ["Dashboard", "Transcript", "Documentation"].index(
                    st.session_state["page"]
                )
                if st.session_state["page"]
                in ["Dashboard", "Transcript", "Documentation"]
                else 0
            ),
            key="nav_radio",
        )
