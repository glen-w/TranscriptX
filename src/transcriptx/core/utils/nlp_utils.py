...
def load_discourse_stoplist():
    path = DISCOURSE_STOPLIST_FILE
    if not path.exists():
        notify_user(
            f"⚠️ Discourse stoplist file not found: {DISCOURSE_STOPLIST_FILE}",
            technical=True,
            section="ner",
        )
        return {}
...
DISCOURSE_STOPLIST_BY_CATEGORY = load_discourse_stoplist()
ALL_DISCOURSE_STOPWORDS = set(
    phrase.lower()
    for phrases in DISCOURSE_STOPLIST_BY_CATEGORY.values()
    for phrase in phrases
)
DISCOURSE_VERBS = set(
    word.lower() for word in DISCOURSE_STOPLIST_BY_CATEGORY.get("discourse_verbs", [])
)
DISCOURSE_HEDGE_TERMS = set(
    word.lower() for word in DISCOURSE_STOPLIST_BY_CATEGORY.get("hedge_terms", [])
)


def build_tic_mask(extra_terms: set[str] | None = None) -> set[str]:
    """Build a unified mask for style/discourse terms."""
    mask = set(ALL_VERBAL_TICS).union(ALL_DISCOURSE_STOPWORDS)
    if extra_terms:
        mask.update(term.lower() for term in extra_terms)
    return mask


def phrase_contains_masked_term(phrase_tokens: list[str], tic_mask: set[str]) -> bool:
    """True when any token in a phrase belongs to the style/discourse mask."""
    return any(token.lower() in tic_mask for token in phrase_tokens)
...
def preprocess_for_insight_eligibility(text: str) -> str:
    """
    Preprocessing optimized for shared insight eligibility filtering.

    Keeps nouns/proper nouns/adjectives and keeps verbs only when they are not
    discourse verbs in the curated discourse stoplist.
    """
    if not text or not text.strip():
        return ""

    doc = _get_nlp_model()(text.lower())
    content_tags = {"NOUN", "PROPN", "ADJ", "VERB", "AUX"}
    mask = build_tic_mask()
    ...
    for token in doc:
        if not token.is_alpha:
            continue
        if token.text in get_all_stopwords():
            continue
        if token.text in mask:
            continue
        if token.pos_ not in content_tags:
            continue
        if token.pos_ in {"VERB", "AUX"} and token.lemma_.lower() in DISCOURSE_VERBS:
            continue
        filtered_tokens.append(token.text)
    return " ".join(filtered_tokens)
