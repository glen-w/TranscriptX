...
def validate_transcript_file(file_path: str) -> bool:
    ...
    the exact structure the pipeline will use. That way segment data
    with only word-level speaker are normalized by the loader
    ...
    try:
        ...
        # Loader fills segment-level "speaker" from words for word-level segment files.
...
def validate_configuration(config: dict[str, Any]) -> bool:
    ...
    # Validate required top-level keys (transcription section removed)
    required_sections = ["analysis", "output", "logging"]
