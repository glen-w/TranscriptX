...
def _get_db_session():
    """Lazy import to avoid circular import (core -> file_rename -> database -> core)."""
    from transcriptx.database import get_session

    return get_session()


def _get_file_tracking_service():
    """Lazy import to avoid circular import."""
    from transcriptx.database import FileTrackingService
...
def rename_transcript_files(
    ...
    try:
        ...
        if not dry_run:
            try:
                session = _get_db_session()
                tracking_service = _get_file_tracking_service()(session)
