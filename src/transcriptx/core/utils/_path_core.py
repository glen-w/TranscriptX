...
Output directory contract (canonical):
  Analysis modules should use ensure_output_dirs(transcript_path, module_name)
  for creating module output directories, either directly or via
  common.create_module_output_structure(). create_standard_output_structure
  in output_standards.py takes transcript_dir (not path) and is for run-level
  layout; ensure_output_dirs is the single source of truth for per-module dirs.
"""
...
def strip_duplicate_filename_suffix(stem: str) -> str:
    """
    Remove trailing copy suffixes that Finder / Explorer add, e.g. ``'foo (1)'`` → ``'foo'``.

    Used when matching transcript JSON names to sibling audio files that kept the
    original base name. Only strips a space plus parentheses around a 1–3 digit
    positive integer (so ``'topic (2024)'`` is left unchanged).
    """
    if not stem:
        return stem
    return re.sub(r" \(([1-9]\d{0,2})\)$", "", stem)
...
def is_canonical_transcript_filename(path: str | Path) -> bool:
    ...
    Canonical filenames end with *_transcriptx.json. All other names are
    non-canonical; analysis will refuse them unless the user runs an import step
    or passes --accept-noncanonical.
    """
    name = os.path.basename(str(path))
    return name.endswith("_transcriptx.json")
...
def get_transcript_dir(transcript_path: str) -> str:
    ...
    return str(Path(OUTPUTS_DIR) / base_name)
...
def get_module_output_dir(transcript_path: str, module_name: str) -> str:
    ...
    return str(Path(transcript_dir) / module_name)
...
def get_group_output_dir(group_uuid: str, run_id: str) -> str:
    ...
    """
    return str(Path(GROUP_OUTPUTS_DIR) / group_uuid / run_id)
...
def get_module_data_file(transcript_path: str, module_name: str, filename: str) -> str:
    ...
    return str(Path(module_dir) / filename)
...
def find_enriched_transcript(transcript_path: str, module_name: str) -> str | None:
    """
    Return the path to an enriched transcript if it exists at the standardized layout.
    ...
    Returns:
        Standardized enriched transcript path if the file exists, else None
    """
...
def ensure_output_dirs(transcript_path: str, module_name: str) -> Dict[str, str]:
    ...
    """
    module_dir = Path(get_module_output_dir(transcript_path, module_name))
    ...
    return {
        ...
        "global_charts_dir": str(global_charts_dir),
        "speaker_data_dir": str(speaker_data_dir),
        "speaker_charts_dir": str(speaker_charts_dir),
    }
...
def get_stats_summary_path(transcript_path: str) -> str:
    ...
    base_name = get_base_name(transcript_path)
    transcript_dir = get_transcript_dir(transcript_path)
    summary_dir = Path(transcript_dir) / "stats" / "summary"
    summary_dir.mkdir(parents=True, exist_ok=True)
    return str(summary_dir / f"{base_name}_comprehensive_summary.txt")
...
def get_html_summary_path(transcript_path: str) -> str:
    ...
    base_name = get_base_name(transcript_path)
    transcript_dir = get_transcript_dir(transcript_path)
    return str(Path(transcript_dir) / f"{base_name}_comprehensive_summary.html")
