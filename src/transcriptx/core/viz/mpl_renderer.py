...
def _all_numeric_zero(values: list[float]) -> bool:
    """Return True when all numeric values are exactly zero."""
    return bool(values) and all(float(v) == 0.0 for v in values)


def _none_detected_message(spec: ChartSpec) -> str | None:
    """Infer a standard no-signal message for empty/zeroed chart payloads."""
    if getattr(spec, "notes", None):
        return None

    if isinstance(spec, HeatmapMatrixSpec) or spec.chart_intent == "heatmap_matrix":
        matrix = getattr(spec, "z", ()) or ()
        flat = [float(v) for row in matrix for v in row]
        if not flat or _all_numeric_zero(flat):
            return "None detected in this transcript."
        return None

    if isinstance(spec, LineTimeSeriesSpec) or spec.chart_intent == "line_timeseries":
        series = list(getattr(spec, "series", ()) or ())
        if not series:
            return "None detected in this transcript."
        y_vals: list[float] = []
        for s in series:
            for y in s.get("y", []) or []:
                try:
                    ...
                    pass
        if y_vals and _all_numeric_zero(y_vals):
            return "None detected in this transcript."
        return None
    ...
    if isinstance(spec, ScatterSpec) or spec.chart_intent in (
        ...
        series_list = spec.get_series() if isinstance(spec, ScatterSpec) else []
        if not series_list:
            return "None detected in this transcript."
        y_vals: list[float] = []
    ...
    if isinstance(spec, BarCategoricalSpec) or spec.chart_intent == "bar_categorical":
        ...
        if getattr(spec, "series", None):
            ...
            for v in getattr(spec, "values", []) or []:
                try:
                    values.append(float(v))
                except (TypeError, ValueError):
                    pass
        if not values or _all_numeric_zero(values):
            return "None detected in this transcript."
        return None
    ...
    if isinstance(spec, NetworkGraphSpec) or spec.chart_intent == "network_graph":
        edges = list(getattr(spec, "edges", ()) or ())
        if not edges:
            return "None detected in this transcript."
        weights = [float(e.get("weight", 0)) for e in edges]
        if _all_numeric_zero(weights):
            return "None detected in this transcript."
        return None
...
def _draw_none_detected(ax: Any, message: str | None) -> None:
    if not message:
        return
    ax.text(
        0.5,
...
def render_mpl(spec: ChartSpec) -> Any:
    ...
    if isinstance(spec, LineTimeSeriesSpec) or spec.chart_intent == "line_timeseries":
        ...
        if any(series.get("name") for series in spec.series):
            # Place legend outside plot (right) so it does not cover the lines
            ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5))
            fig.tight_layout(rect=[0, 0, 0.85, 1])
        else:
            fig.tight_layout()
        ax.grid(True, alpha=0.3)
        _draw_none_detected(ax, _none_detected_message(spec))
        return fig
    ...
    if isinstance(spec, HeatmapMatrixSpec) or spec.chart_intent == "heatmap_matrix":
        ...
        z_values = [float(v) for row in spec.z for v in row]
        zmin = spec.zmin
        zmax = spec.zmax
        if (
            z_values
            and zmin is None
            and zmax is None
            and min(z_values) == max(z_values)
        ):
            # Keep colorbar semantically readable for constant matrices (e.g. all zeros).
            zmin = min(z_values)
            zmax = zmin + 1.0
        ...
        _draw_none_detected(ax, spec.notes or _none_detected_message(spec))
    ...
    if isinstance(spec, BoxSpec) or spec.chart_intent == "box_plot":
        ...
        for series in series_list:
            ...
            for x in xs:
                if x not in box_categories:
                    box_categories.append(x)
        ...
        for idx, series in enumerate(series_list):
            ...
            if xs:
                for x_val, y_val in zip(xs, ys):
                    if x_val in box_categories:
                        grouped[box_categories.index(x_val)].append(y_val)
            else:
