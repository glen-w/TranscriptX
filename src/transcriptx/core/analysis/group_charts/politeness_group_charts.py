"""Politeness: session bars plus pooled category counts."""
...
class PolitenessGroupChartGenerator:
    agg_id = "politeness"

    def __init__(self) -> None:
        allow = allowed_numeric_keys_for_generic_agg("politeness")
    ...
    def can_generate(self, outcome: Dict[str, Any]) -> bool:
        if self._session.can_generate(outcome):
            return True
        p = outcome.get("politeness_pooled")
    ...
    def generate(
        ...
        if isinstance(p, dict):
            ...
            if isinstance(by_cat, dict) and by_cat:
                ...
                svc.save_chart(
                    BarCategoricalSpec(
                        viz_id="group.politeness.pooled.by_category.global",
                        module=self.agg_id,
                        name="politeness_pooled_by_category",
                        scope="global",
                        chart_intent="bar_categorical",
                        title="Group pooled — politeness marker counts",
                        x_label="Category",
                        y_label="Count",
