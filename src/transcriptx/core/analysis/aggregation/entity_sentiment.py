...
def aggregate_entity_sentiment_group(
    ...
    pooled_entities = []
    for (entity, entity_type), entry in global_agg.items():
        m = entry["mentions"] or 1
        pooled_entities.append(
            {
                "entity": entity,
                "entity_type": entity_type,
                "mentions": int(entry["mentions"]),
                "mean_sentiment": entry["sum_compound"] / m,
                "pos": entry["sum_pos"] / m,
                "neu": entry["sum_neu"] / m,
                "neg": entry["sum_neg"] / m,
            }
        )
    pooled_entities.sort(key=lambda r: -r["mentions"])
    entity_sentiment_pooled = {
        "schema_version": 1,
        "entities": pooled_entities[:50],
    }

    return {
        "session_rows": session_rows,
        "speaker_rows": speaker_rows,
        "entity_sentiment_pooled": entity_sentiment_pooled,
    }
