...
from transcriptx.core.analysis.aggregation.common import (
    build_rows_from_stats as _build_rows_from_stats_common,
    extract_payload as _extract_payload_common,
    warning_payload_shape as _warning_payload_shape_common,
)
...
def _extract_payload(
    ...
    return _extract_payload_common(module_results, module_name)
...
def _build_rows_from_stats(
    ...
    return _build_rows_from_stats_common(
...
def _warning_payload_shape(agg_id: str, expected_keys: List[str]) -> Dict[str, Any]:
    return _warning_payload_shape_common(agg_id, expected_keys)
...
def _aggregate_wordclouds(
    ...
    if grouped:
        ...
        if summary is not None:
            ...
            if p:
                summary["pooled_cross_session_summary_path"] = p
            sk = wc_extra.get("skipped_variants") or []
            if sk:
                prev = summary.get("skipped_variants") or []
                summary["skipped_variants"] = list(prev) + list(sk)
    if summary is None:
        return None
    return {
        "session_rows": [
...
def _aggregate_tics(
    ...
    for result in per_transcript_results:
        ...
        tt = global_stats.get("total_tics")
        if isinstance(tt, (int, float)):
            total_tics += int(tt)
        for k, v in global_stats.items():
            if k == "total_tics" or not isinstance(v, (int, float)):
                continue
            by_tic[k] = by_tic.get(k, 0) + int(v)
        rows = _build_rows_from_stats(
            result, transcript_set, canonical_speaker_map, global_stats, speaker_stats
        )
        session_rows.extend(rows["session_rows"])
    ...
    tics_pooled = {
        ...
        "by_tic": dict(sorted(by_tic.items(), key=lambda kv: -kv[1])),
    }
    return {
        "session_rows": session_rows,
        "speaker_rows": speaker_rows,
        "tics_pooled": tics_pooled,
    }
...
def _aggregate_contagion(
    ...
    if not session_rows:
        return None
    pooled, agg_warns = build_contagion_pooled_for_group(
        per_transcript_results, canonical_speaker_map
    )
    out: Dict[str, Any] = {
        "session_rows": session_rows,
        "speaker_rows": speaker_rows,
        "contagion_pooled": pooled,
    }
    if agg_warns:
        out["aggregation_warnings"] = agg_warns
    return out
...
def build_registry() -> List[AggregationEntry]:
    ...
    from transcriptx.core.analysis.aggregation.bertopic import aggregate_bertopic_group
    ...
    registry: List[AggregationEntry] = [
        ...
        AggregationEntry(
            agg_id="bertopic",
            selector=any_of(["bertopic"]),
            deps=[],
            aggregate_fn=aggregate_bertopic_group,
        ),
        AggregationEntry(
            agg_id="emotion",
