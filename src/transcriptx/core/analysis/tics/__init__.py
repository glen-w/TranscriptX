...
class TicsAnalysis(AnalysisModule):
    ...
    def analyze(self, segments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for seg in segments:
            ...
            # Include multi-word verbal tics so downstream mask semantics are consistent.
            for tic_phrase in extract_tics_from_text(text):
                if " " in tic_phrase:
                    tic_counts[speaker][tic_phrase] += 1
