...
class TemporalDynamicsAnalysis(AnalysisModule):
    ...
    def _create_time_series_plots(
        ...
        # Speaking rate is emitted once in _create_dashboard as "Temporal Dynamics – Speaking Rate"
        # (same data; avoid duplicate chart)
    ...
    def _create_dashboard(
        ...
        x_display, x_label = time_axis_display(window_centers_sec)
        # Only Turn Frequency and Num Segments (similar scale). Engagement is 0–1 so excluded
        # to keep the Y-axis readable; it has its own "Engagement Over Time" chart.
        series_metrics = [
            {
                "name": "Turn Frequency",
                "x": x_display,
