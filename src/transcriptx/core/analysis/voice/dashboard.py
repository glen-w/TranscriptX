...
from transcriptx.core.analysis.voice.prosody_overlay_segments import (
    PROSODY_OVERLAY_SEGMENTS_FILESTEM,
...
def _prepare_data(context: Any, locator: Dict[str, Any]) -> _ProsodyData:
    optional_import("pandas", "prosody dashboard")
...
class ProsodyDashboardAnalysis(AnalysisModule):
    ...
    def analyze(self, segments) -> Dict[str, Any]:
        return {}
    ...
    def run_from_context(self, context: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if "start_s" in df.columns and "rms_db" in df.columns and not df.empty:
                seg_payload = build_prosody_overlay_segments_v1_payload(
                    df.to_dict("records")
                )
                if seg_payload["segments"]:
                    output_service.save_data(
                        seg_payload,
                        PROSODY_OVERLAY_SEGMENTS_FILESTEM,
                        format_type="json",
