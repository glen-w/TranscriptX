"""Epistemic markers (hedging / certainty) analysis module — B6."""
...
from transcriptx.core.utils.viz_ids import VIZ_EPISTEMIC_MARKERS_CATEGORY_SPEAKER
...
SCHEMA_ID = "transcriptx.epistemic_markers.v1"
...
class EpistemicMarkersAnalysis(AnalysisModule):
    """Lexicon-first hedging / certainty / epistemic marker density."""

    def __init__(self, config: Dict[str, Any] | None = None) -> None:
        super().__init__(config)
        self.module_name = "epistemic_markers"

    def _settings(self) -> tuple[list[str] | None, int]:
        try:
            from transcriptx.core.utils.config import get_config

            cfg = get_config().analysis.epistemic_markers
    ...
    def _save_results(
        ...
        output_service.save_data(payload, "epistemic_markers", format_type="json")
        csv_path = Path(output_structure.global_data_dir) / (
            f"{base_name}_epistemic_markers.csv"
        )
    ...
    def _save_charts(
        ...
        for speaker, stats in (results.get("speaker_stats") or {}).items():
            ...
            spec = BarCategoricalSpec(
                viz_id=VIZ_EPISTEMIC_MARKERS_CATEGORY_SPEAKER,
                module=self.module_name,
                name="category_rates",
                scope="speaker",
                speaker=speaker,
                chart_intent="bar_categorical",
                title=f"Epistemic markers: {speaker}",
                x_label="Category",
                y_label="Rate per 100 tokens (or count)",
                categories=labels,
                values=values,
            )
            output_service.save_chart(spec, chart_type="epistemic_markers")
