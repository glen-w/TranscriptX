...
try:
    ...
    # Fallback for modules that haven't been migrated yet (runtime only)
    PipelineContext = None  # type: ignore[assignment,misc]
    OutputService = None  # type: ignore[assignment,misc]
    create_output_service = None  # type: ignore[assignment,misc]
...
class ExampleAnalysisModule(AnalysisModule):
    ...
    def analyze(self, segments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_speakers = len(get_unique_speakers(segments))
