"""Group aggregation for politeness."""
...
def aggregate_politeness(
    ...
    soft_ratios: List[float] = []

    for result in per_transcript_results:
        payload = _extract_payload(result.module_results, "politeness")
        if not payload:
            continue
        ...
        if not isinstance(global_stats, dict) or not isinstance(speaker_stats, dict):
            return _warning_payload_shape(
                "politeness", ["global_stats", "speaker_stats"]
            )
    ...
    mean_soft = sum(soft_ratios) / len(soft_ratios) if soft_ratios else None
    pooled = {
        "schema_version": 1,
        "total_marker_hits": total_hits,
        "by_category": dict(sorted(by_category.items(), key=lambda kv: -kv[1])),
        "mean_hits_per_100_tokens": mean_rate,
        "mean_soft_request_ratio": mean_soft,
        "rate_pooling_note": (
            "Mean rates are descriptive session means, not exact token-weighted pools. "
            "Interactional power (floor/interruption) remains in interactions equity."
        ),
    }
    return {
        "session_rows": session_rows,
        "speaker_rows": speaker_rows,
        "politeness_pooled": pooled,
    }
