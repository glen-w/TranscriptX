...
def generate_acts_charts(
    ...
    *,
    ...
    group_aggregate_viz_ids: bool = False,
) -> None:
    """Generate all acts charts (pie, bar, timeline per speaker and global) via output_service."""

    def _vid(viz_id: str) -> str:
        if not group_aggregate_viz_ids:
            return viz_id
        group_map = {
            "acts.acts_pie.speaker": "group.acts.acts_pie.speaker",
            "acts.acts_bar.speaker": "group.acts.acts_bar.speaker",
            "acts.global_acts_pie.global": "group.acts.global_acts_pie.global",
            "acts.global_acts_bar.global": "group.acts.global_acts_bar.global",
            "acts.acts_temporal_all.global": "group.acts.acts_temporal_all.global",
            "acts.acts_temporal.speaker": "group.acts.acts_temporal.speaker",
        }
        return group_map.get(viz_id, viz_id)
    ...
    for speaker, counter in act_counts_per_speaker.items():
        ...
        if acts_filt:
            output_service.save_chart(
                BarCategoricalSpec(
                    viz_id=_vid("acts.acts_pie.speaker"),
                    module="acts",
                    name="acts_pie",
                    scope="speaker",
                    speaker=speaker,
                    chart_intent="bar_categorical",
                    title=_tp(f"Dialogue Acts (Filtered) – {speaker}"),
                    x_label="Act Type",
                    y_label="Count",
        ...
        if acts:
            output_service.save_chart(
                BarCategoricalSpec(
                    viz_id=_vid("acts.acts_bar.speaker"),
                    module="acts",
                    name="acts_bar",
                    scope="speaker",
                    speaker=speaker,
                    chart_intent="bar_categorical",
                    title=_tp(f"Dialogue Acts – {speaker}"),
                    x_label="Act Type",
                    y_label="Count",
                    categories=acts,
                    values=counts,
    ...
    if act_counts_global:
        ...
        if acts_filt:
            output_service.save_chart(
                BarCategoricalSpec(
                    viz_id=_vid("acts.global_acts_pie.global"),
                    module="acts",
                    name="global_acts_pie",
                    scope="global",
                    chart_intent="bar_categorical",
                    title=_tp("Dialogue Acts (Filtered) – All Speakers"),
                    x_label="Act Type",
                    y_label="Count",
                    categories=acts_filt,
                    values=counts_filt,
    ...
    if not speakers:
        ...
        output_service.save_chart(
            BarCategoricalSpec(
                viz_id=_vid("acts.global_acts_bar.global"),
                module="acts",
                name="global_acts_bar",
                scope="global",
                chart_intent="bar_categorical",
                title=_tp("Dialogue Acts by Speaker"),
    ...
    if act_counts_global and speakers:
        ...
        if temporal_series:
            output_service.save_chart(
                LineTimeSeriesSpec(
                    viz_id=_vid("acts.acts_temporal_all.global"),
                    module="acts",
                    name="acts_temporal_all",
                    scope="global",
                    chart_intent="line_timeseries",
                    title=_tp("Dialogue Acts Over Time – All Speakers"),
    ...
    for speaker in speakers:
        ...
        output_service.save_chart(
            LineTimeSeriesSpec(
                viz_id=_vid("acts.acts_temporal.speaker"),
                module="acts",
                name="acts_temporal",
                scope="speaker",
                speaker=speaker,
                chart_intent="line_timeseries",
                title=_tp(f"Dialogue Acts Over Time – {speaker}"),
                x_label="Time (minutes)",
                y_label="Dialogue Act (index)",
                markers=True,
...
def tag_acts(
    ...
    for i, seg in enumerate(segments):
        ...
        # Use segment-based speaker extraction
        from transcriptx.core.utils.speaker_extraction import (
            extract_speaker_info,
            get_speaker_display_name,
        )
...
def _generate_comparison_summary(
    ...
    for act, count in ml_acts.most_common():
        percentage = (count / total_utterances) * 100 if total_utterances > 0 else 0.0
