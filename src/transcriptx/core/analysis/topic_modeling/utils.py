...
def prepare_text_data(
    ...
    Uses segment-based speaker identification via extract_speaker_info().
...
def prepare_text_data_from_windows(
    eligibility_result: dict[str, Any] | None,
) -> tuple[list[str], list[str], list[float]]:
    """Prepare canonical topic documents from rolling windows."""
    if not isinstance(eligibility_result, dict):
        return [], [], []
    windows = eligibility_result.get("windows") or []
    texts: list[str] = []
    ...
    for window in windows:
        ...
        if isinstance(speakers, list) and speakers:
            speaker_labels.append(" / ".join(sorted({str(s) for s in speakers if s})))
        else:
            speaker_labels.append("window")
        time_labels.append(float(window.get("start", 0.0) or 0.0))
    return texts, speaker_labels, time_labels
...
def _normalize_term_for_label(term: str) -> str:
...
def generate_topic_labels(
    ...
    banned = {term.lower() for term in (banned_terms or set())}
    weighted = list(zip(top_words, weights or [1.0] * len(top_words), strict=False))
    weighted.sort(key=lambda pair: (-float(pair[1]), str(pair[0]).lower()))

    selected: list[str] = []
    seen_lemmas: set[str] = set()
    seen_stems: set[str] = set()
    for word, _weight in weighted:
        term = str(word).strip().lower()
        if not term or term in banned:
            continue
        lemma = _normalize_term_for_label(term)
        stem = lemma[:4] if len(lemma) > 4 else lemma
        if lemma in seen_lemmas or stem in seen_stems:
            continue
...
def topic_rejected(
    ...
    *,
    ...
    """Fail-closed topic filter based on banned term concentration."""
    if not top_terms:
        return True
    banned = {term.lower() for term in (banned_terms or build_tic_mask())}
    lowered = [term.lower() for term in top_terms]
    if lowered[0] in banned:
        return True
    banned_count = sum(1 for term in lowered if term in banned)
    return (float(banned_count) / float(len(lowered))) > threshold
