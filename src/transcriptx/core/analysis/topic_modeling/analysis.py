...
from .utils import (
    prepare_text_data,
    prepare_text_data_from_windows,
...
class TopicModelingAnalysis(AnalysisModule):
    ...
    def run_from_context(self, context):
        self._eligibility_result = context.get_analysis_result("insight_eligibility")
        try:
    ...
    def analyze(
        ...
        # Canonical input representation: rolling windows from insight_eligibility.
        texts, speaker_labels, time_labels = prepare_text_data_from_windows(
            self._eligibility_result
        )
        if not texts:
            # Backward-compatible fallback for runs without insight_eligibility.
            import warnings

            warnings.warn(
                "Topic modeling fell back to segment-level text because insight_eligibility "
                "windows were missing or empty; canonical runs should supply eligibility.",
                UserWarning,
                stacklevel=2,
            )
        ...
        tic_mask = set(
            (self._eligibility_result or {}).get("tic_mask") or build_tic_mask()
        )
        ...
        lda_results = perform_enhanced_lda_analysis(
            texts,
            speaker_labels,
            time_labels,
            extra_stopwords=tic_mask,
        )
        nmf_results = perform_enhanced_nmf_analysis(
            texts,
            speaker_labels,
            time_labels,
            extra_stopwords=tic_mask,
        ...
        nmf_rejected = []
        lda_topics_filtered = []
        for topic in lda_results.get("topics", []):
            if topic_rejected(
                topic.get("words", []), banned_terms=tic_mask, threshold=0.5
            ):
                lda_rejected.append({**topic, "rejected": True})
                continue
            lda_topics_filtered.append(topic)
        nmf_topics_filtered = []
        for topic in nmf_results.get("topics", []):
            if topic_rejected(
                topic.get("words", []), banned_terms=tic_mask, threshold=0.5
            ):
                nmf_rejected.append({**topic, "rejected": True})
                continue
            nmf_topics_filtered.append(topic)
        lda_results["topics"] = lda_topics_filtered
        nmf_results["topics"] = nmf_topics_filtered
    ...
    def _save_results(
        ...
        rejected_topics = results.get("rejected_topics", {})
        ...
        if rejected_topics:
            output_service.save_data(
                rejected_topics, "rejected_topics_debug", format_type="json"
            )
