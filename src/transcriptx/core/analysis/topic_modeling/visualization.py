...
def create_enhanced_global_heatmaps(
    ...
    try:
        ...
        ax.imshow(topic_word_matrix, cmap="Blues", aspect="auto")
    ...
    try:
        ...
        topic_word_matrix = np.asarray(topic_word_matrix, dtype=np.float64)
        ax.imshow(topic_word_matrix, cmap="Reds", aspect="auto")
...
def _parse_speaker_names(label: Any) -> list[str]:
    """
    Parse a speaker label into unique speaker names.

    Supports canonical labels like "Ana / Fede" while being defensive about
    whitespace and duplicates (e.g. "Ana / Fede / Ana" -> ["Ana", "Fede"]).
    """
    if label is None:
        return []
    raw = str(label).strip()
...
def _row_weight(row: dict[str, Any]) -> float:
    """
    Resolve row-level weight if present, else default to 1.0.

    This ensures speaker attribution composes with any upstream weighting.
    """
    for key in ("weight", "row_weight", "window_weight"):
        value = row.get(key)
        if value is None:
            continue
        try:
...
def create_speaker_topic_engagement_heatmap(
    ...
    try:
        ...
        if "dominant_topic" not in df or "speaker" not in df:
            ...
            return None
        # Semantic contract:
        # This chart uses fractional attribution of multi-speaker rows.
        # Each source row contributes total weight equal to its row weight
        # (default 1.0) and splits that evenly across unique parsed speakers.
        # Speaker ordering is alphabetical for stable, reproducible output.
        speaker_topic_weights = defaultdict(lambda: defaultdict(float))
        topic_ids: set[int] = set()
        contributing_rows = 0

        for row in df.to_dict(orient="records"):
            if "dominant_topic" not in row or pd.isna(row["dominant_topic"]):
                continue
            speakers = _parse_speaker_names(row.get("speaker"))
            if not speakers:
                continue
            try:
            ...
            share = total_weight / len(speakers)
            for speaker in speakers:
                speaker_topic_weights[speaker][topic_id] += share
            topic_ids.add(topic_id)
            contributing_rows += 1

        if not speaker_topic_weights or not topic_ids:
            print(
                "[TOPICS] Not enough valid rows for speaker-topic engagement heatmap."
            )
            return None

        speaker_labels = sorted(speaker_topic_weights.keys())
        topic_labels_raw = sorted(topic_ids)
        z_matrix = [
            [
                speaker_topic_weights[speaker].get(topic_id, 0.0)
                for topic_id in topic_labels_raw
            ]
            for speaker in speaker_labels
        ]
        print(
            "[DEBUG] speaker-topic engagement heatmap: "
            f"contributors={contributing_rows}, matrix_sum={float(np.sum(z_matrix)):.6f}"
        )
        if output_service:
            label_map = _build_topic_label_map(lda_topics)
            spec = HeatmapMatrixSpec(
                ...
                x_labels=[
                    ...
                    for t in topic_labels_raw
                ],
                y_labels=speaker_labels,
            )
            result = output_service.save_chart(spec, chart_type="heatmap")
            if result.get("static"):
                print(
