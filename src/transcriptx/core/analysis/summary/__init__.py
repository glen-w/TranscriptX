...
class SummaryAnalysis(AnalysisModule):
    ...
    def analyze(
        ...
        highlights["transcript_key"] = "unknown"
        attach_themes_to_highlights(highlights)
    ...
    def run_from_context(self, context: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            _ensure_highlights_themes(highlights_result, context)
...
def _ensure_highlights_themes(highlights: Dict[str, Any], context: Any) -> None:
    """Ensure transcript_key and themes exist for summary / stable quote IDs."""
    if not highlights:
        return
    if not highlights.get("transcript_key"):
        highlights["transcript_key"] = context.get_transcript_key() or "unknown"
    if "themes" not in highlights:
        attach_themes_to_highlights(highlights)
...
def _resolve_highlights(
    ...
    if summary_cfg.compute_highlights_if_missing:
        computed = cast(Dict[str, Any], compute_highlights(normalized, highlights_cfg))
        computed["transcript_key"] = context.get_transcript_key() or "unknown"
        attach_themes_to_highlights(computed)
        return computed, "computed_by_summary"
