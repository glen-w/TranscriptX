...
from .report_renderer import md_to_txt, render_report_md
from .stats_report import build_report_payload
...
class StatsAnalysis(AnalysisModule):
    ...
    def analyze(
        ...
        for seg in segments:
            ...
            fallback_speaker_id = str(seg.get("speaker") or "").strip()
            if speaker_info is None:
                name = fallback_speaker_id
                speaker_id = fallback_speaker_id
            else:
                name = get_speaker_display_name(
                    speaker_info.grouping_key, [seg], segments
                )
                speaker_id = str(speaker_info.grouping_key)
            if not name:
                continue
            if ignored_ids and (
                str(speaker_id) in ignored_ids or str(name) in ignored_ids
            ):
                continue
            fallback_grouped.setdefault(name, []).append(seg.get("text", ""))
            if not is_eligible_named_speaker(
                name, str(speaker_id), ignored_ids or set()
            ):
                continue
            grouped.setdefault(name, []).append(seg.get("text", ""))

        # If every speaker is anonymized (e.g., SPEAKER_00), retain fallback grouping
        # so report totals are still meaningful for unmapped transcripts.
        if not grouped and fallback_grouped:
            logger.info(
                "[STATS] No eligible named speakers found; falling back to all non-ignored speakers"
            )
        ...
        speaker_stats, sentiment_summary = compute_speaker_stats(
            grouped, segments, None, tic_list, ignored_ids=ignored_ids
        )
    ...
    def run_from_context(self, context: "PipelineContext") -> Dict[str, Any]:
        ...
        try:
            ...
            # Create output service for artifact registration
            ...
            # Build report payload (report.v1, resolver-driven)
            ...
            write_text(report_json_path, report_json_text)
            report_md_content = render_report_md(report_payload)
            write_text(report_md_path, report_md_content)
            write_text(report_txt_path, md_to_txt(report_md_content))

            # Explicitly register run-root artifacts for manifest metadata
            output_service._record_artifact_metadata(
                report_json_path,
                {"scope": "global", "format": "json", "title": "User report (JSON)"},
            )
            output_service._record_artifact_metadata(
                report_md_path,
                {"scope": "global", "format": "md", "title": "User report (MD)"},
            )
            output_service._record_artifact_metadata(
                report_txt_path,
                {"scope": "global", "format": "txt", "title": "User report (TXT)"},
            )
            ...
            return {
                ...
                "status": "success",
                "results": results,
                # Stats report artifacts are written at run root.
