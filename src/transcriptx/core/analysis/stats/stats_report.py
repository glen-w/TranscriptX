...
# Short labels for report summary index (section_name, descriptor).
_REPORT_SECTION_LABELS: Dict[str, Tuple[str, str]] = {
    "sentiment": ("Sentiment", "Sentiment overview and tone"),
    "emotion": ("Emotion", "Emotional tone by speaker"),
    "acts": ("Dialogue acts", "Dialogue act distribution"),
    "interactions": ("Speaker dynamics", "Turn-taking and interactions"),
    "ner": ("Named entities", "Key people, places, and things"),
    "entity_sentiment": ("Entity framing", "Entity sentiment framing"),
    "conversation_loops": ("Conversation loops", "Recurring loop patterns"),
    "contagion": ("Contagion", "Emotional contagion"),
    "wordclouds": ("Word frequency", "Top words by speaker"),
    "tics": ("Verbal tics", "Filler words and tics"),
    "understandability": ("Understandability", "Readability scores"),
    "temporal_dynamics": ("Temporal dynamics", "Phases and momentum over time"),
    "pauses": ("Pauses", "Pause and pacing"),
    "momentum": ("Momentum", "Flow and stall indices"),
    "highlights": ("Highlights", "Notable moments"),
    "summary": ("Executive summary", "Brief summary"),
    "affect_tension": ("Affect tension", "Affect and tension"),
}
...
def build_report_payload(
    ...
    *,
    ...
    """
    Build report.v1 payload using the report input resolver.

    Uses run_results.json for module execution state and resolver for
    report_contribution_status (full_section / mention_only / omitted).
    Section payloads included only when resolver reports full_section.
    """
    base_name = context.get_base_name()
    transcript_dir = context.get_transcript_dir()
    run_dir = run_dir or transcript_dir
    run_path = Path(run_dir)

    resolver_results, resolver_warnings = resolve_report_inputs(run_path, base_name)

    # Reuse same provenance, overview, speakers, insights, sentiment as stats payload
    ...
    # Override schema and modules with resolver-driven data
    payload["meta"]["schema_version"] = "report.v1"

    modules_report: Dict[str, Dict[str, Any]] = {}
    report_summary_index: List[Dict[str, Any]] = []

    for module_id, resolution in resolver_results.items():
        entry: Dict[str, Any] = {
            "module_status": resolution.module_status,
            "report_contribution_status": resolution.report_contribution_status,
        }
        if resolution.reason:
            entry["reason"] = resolution.reason
        if resolution.best_input_path:
            entry["best_input_path"] = resolution.best_input_path
        if (
            resolution.report_contribution_status == REPORT_CONTRIBUTION_FULL_SECTION
            and resolution.parsed_data
        ):
            entry["section_payload"] = resolution.parsed_data
            label, descriptor = _REPORT_SECTION_LABELS.get(
                module_id, (module_id.replace("_", " ").title(), "")
            )
            report_summary_index.append(
                {
                    "section_name": label,
                    "descriptor": descriptor or f"Summary from {module_id}",
                    "source_modules": [module_id],
                }
            )
        modules_report[module_id] = entry

    payload["modules"] = modules_report
    payload["report_summary_index"] = report_summary_index
    payload["warnings"] = list(payload.get("warnings") or []) + list(resolver_warnings)
    # report.json is not a mini-manifest: drop artifact list
    payload.pop("outputs_index", None)
...
def _build_modules_payload(
    ...
    run_results, run_results_note = _load_run_results_optional(Path(transcript_dir))
...
def _resolve_module_status(
    ...
    run_results_note: str | None = None,
) -> Tuple[str, str, List[str]]:
    outputs = _existing_outputs(transcript_dir, base_name, spec.expected_outputs)
    if run_results is not None:
        canonical = status_for_module(
            spec.module_id, run_results, default_if_missing="requested"
        )
        if canonical == "succeeded":
            if outputs:
                return "succeeded", "canonical succeeded", outputs
            return "succeeded", "canonical succeeded, outputs missing", []
        if canonical in {"blocked", "skipped", "failed", "enabled", "requested"}:
            return canonical, f"canonical {canonical}", outputs
    # run_results absent or unloadable: status is unknown (not inferred from disk as success).
    note = f" [{run_results_note}]" if run_results_note else ""
    if outputs:
        return (
            "unknown",
            "execution status unknown without run_results.json; "
            f"outputs on disk (diagnostic only, not status truth){note}",
            outputs,
        )
    return (
        "unknown",
        "no run_results and no outputs" + note,
        [],
    )


def _load_run_results_optional(
    run_dir: Path,
) -> tuple[Dict[str, Any] | None, str | None]:
    path = run_dir / "run_results.json"
    if not path.exists():
        return None, "run_results.json missing"
    try:
        return load_run_results(path), None
    except Exception as exc:
        return None, f"run_results load failed: {type(exc).__name__}: {exc}"
