...
def aggregate_stats_group(
    ...
    stats_pooled = {
        "schema_version": 1,
        "total_words": sum(int(row.get("total_words", 0) or 0) for row in session_rows),
        "total_segments": sum(
            int(row.get("total_segments", 0) or 0) for row in session_rows
        ),
        "total_duration": sum(
            float(row.get("total_duration", 0) or 0) for row in session_rows
        ),
    }
