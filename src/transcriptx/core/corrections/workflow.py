...
def _derive_speaker_map(
    ...
    speaker_map_metadata = SpeakerMapResolver().load_mapping(transcript_path)
    if speaker_map_metadata.speaker_map:
        set_speaker_display_map(speaker_map_metadata.speaker_map)
...
def run_corrections_on_segments(
    *,
    ...
    # When decisions are supplied externally (e.g. from Corrections Studio),
    # skip interactive review and auto-apply filtering entirely.
    decisions_path_saved: Optional[str] = None
    if decisions is not None:
        apply_candidates = candidates
    elif interactive_review:
