...
def _infer_tags(module: Optional[str], rel_path: str, parts: List[str]) -> List[str]:
    ...
    if "ner" in lower_name or "entity" in lower_name:
        tags.add("entities")
    if "insight_eligibility" in lower_name or module == "insight_eligibility":
        tags.add("eligibility")
    if "tic_mask" in lower_name:
        tags.add("tic_mask")
    if "phrase_scores" in lower_name:
        tags.add("phrase_scores")
    return sorted(tags)
    ...
    for path in _iter_files(run_dir):
        ...
        if isinstance(meta, dict):
            ...
            if isinstance(meta_tags, list) and meta_tags:
                tags = sorted(set(tags) | {str(t) for t in meta_tags if t is not None})
        produced_by = None
