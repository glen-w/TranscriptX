...
def run_analysis_pipeline(
    ...
    """
    Run the analysis pipeline on a transcript or TranscriptSet.
    ...
    ensure_data_dirs()
    resolved_paths, transcript_set = resolve_analysis_target(target)
    if len(resolved_paths) == 1 and transcript_set is None:
    ...
    if transcript_set is None:
        transcript_set = TranscriptSet.create(transcript_ids=resolved_paths)
    ...
    # Phase 2: group aggregation (stats pilot)
    ...
    if group_config.group_analysis.persist_groups:
        try:
            from transcriptx.database import get_session
            from transcriptx.database.repositories.transcript_set import (
            ...
            session = get_session()
            try:
                set_repo = TranscriptSetRepository(session)
                existing = set_repo.get_by_key(transcript_set.key)
                if existing:
                    logger.info(
                    ...
                    transcript_file_ids = []
                    for transcript_id in transcript_set.transcript_ids:
                        record = file_repo.get_transcript_file_by_path(transcript_id)
                        if record is not None:
                            transcript_file_ids.append(record.id)
                    set_repo.create_transcript_set(
                        name=transcript_set.name,
                        transcript_ids=transcript_set.transcript_ids,
                        metadata=transcript_set.metadata,
                        transcript_file_ids=transcript_file_ids or None,
                    )
            finally:
                session.close()
        except Exception as exc:
            logger.warning("Failed to persist TranscriptSet: %s", exc)
    ...
    if persist:
        try:
            ...
            pipeline_config = {
                "modules": selected_modules,
                "analysis_mode": (
                    getattr(config, "analysis", {}).get("analysis_mode", "quick")
                    if config is not None
                    else "quick"
                ),
                "quality_profile": (
                    getattr(config, "analysis", {}).get(
...
def get_default_modules() -> List[str]:
    ...
    return list(get_default_modules_from_registry())
