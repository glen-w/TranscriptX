...
class RunManifestInput(BaseModel):
    """
    Canonical input for the analysis pipeline. Callers (Python API, manifest JSON)
    ...
    def from_cli_kwargs(
        ...
        """Build RunManifestInput from explicit keyword arguments (mirrors manifest fields)."""
...
class ModuleSkippedEntry(BaseModel):
    """One skipped or blocked module with reason."""

    module: str
    reason: str
    execution_status: str = Field(
        default="skipped",
        description="skipped = intentional non-run; blocked = dependency/precondition",
    )
    ...
    def validate_run_results(cls, data: Dict[str, Any]) -> "RunResultsSummary":
        ...
        for s in skipped:
            if not isinstance(s, dict):
                raise ValueError(
                    "modules_skipped entries must be dicts with module, reason, execution_status"
                )
            st = s.get("execution_status", "skipped")
...
def validate_manifest_shape(manifest: Dict[str, Any]) -> None:
    ...
    if manifest.get("manifest_type") != MANIFEST_TYPE_ARTIFACT:
        raise ValueError(
