...
from transcriptx.core.pipeline.module_outcomes import project_failed_modules
...
class OutputReporter:
    ...
    def generate_comprehensive_output_summary(
        ...
        modules_run_set = {canonical_module_id(m) for m in modules_run}

        def _ran(mod: str) -> bool:
            return canonical_module_id(mod) in modules_run_set

        skipped_by_canonical: Dict[str, Dict[str, Any]] = {}
        for entry in skipped_modules or []:
            if isinstance(entry, dict) and entry.get("module") is not None:
                skipped_by_canonical[canonical_module_id(str(entry["module"]))] = entry

        modules_failed = project_failed_modules(
            selected_modules, modules_run, skipped_modules
        )
        ...
        # Scan for all outputs (execution truth: modules_run drives success bucket)
        for module_name in selected_modules:
            module_dir = _get_module_output_dir(basename_dir, module_name)
            in_run = _ran(module_name)
            sk = skipped_by_canonical.get(canonical_module_id(module_name))
            ...
            if in_run:
                ...
                if module_dir.exists():
                    ...
                    module_outputs["artifact_note"] = (
                        "Output directory missing; module still recorded as executed."
                    )
                ...
                summary["outputs"]["skipped_modules"][module_name] = {
                    ...
                    "reason": sk.get("reason", "Skipped"),
                    "execution_status": sk.get("execution_status", "skipped"),
                }
            else:
    ...
    def print_compact_post_run_summary(
        ...
        group_phase = results.get("group_phase_metadata")
        if isinstance(group_phase, dict) and group_phase.get(
        ...
        failed_modules = project_failed_modules(
            list(requested), list(modules_run), skipped_raw
        )
        self.console.print("\n---")
