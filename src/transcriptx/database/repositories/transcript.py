...
class TranscriptFileRepository(BaseRepository):
    ...
    def get_transcript_file_by_file_name(
        self, file_name: str
    ) -> Optional[TranscriptFile]:
        """
        Get transcript file by file name (basename) when exactly one record matches.
        Returns None if zero or multiple records have this file_name (avoids ambiguity).
        """
        try:
            matches = (
                self.session.query(TranscriptFile)
                .filter(TranscriptFile.file_name == file_name)
                .all()
            )
            return matches[0] if len(matches) == 1 else None
        except Exception as e:
            self._handle_error("get_transcript_file_by_file_name", e)
