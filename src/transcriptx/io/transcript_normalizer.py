...
class TranscriptNormalizer:
    ...
    def _repair_end_timestamps(
        ...
        for i, turn in enumerate(turns):
            invalid_order = (
                turn.start is not None
                and turn.end is not None
                and turn.end <= turn.start
            )
            if turn.end is None or invalid_order:
                ...
                if turn.end is None:
                    ...
                    warnings.append(
                        f"Turn {i}: end ({turn.end:.3f}) <= start "
                        f"({(turn.start or 0.0):.3f}); repaired to {new_end:.3f}s"
                    )
