...
def _is_success_exit_code(exit_code: object) -> bool:
    """True if exit_code indicates success (0); handles int, str, numpy, etc."""
        ...
        if audio_path:
            ...
            if proc is not None:
                ...
                if proc.poll() is None:  # Process is still running
                    ...
                    status = f"▶️ Playing ({seg.start:.1f}s - {seg.end:.1f}s)"
                    if player.last_export_path:
                        status += f" | Saved: {player.last_export_path.name}"
                    update_status(status)
                else:
                    # Process already exited (e.g. short segment finished, or ffplay -autoexit, or no audio in Docker)
                    exit_code = (
                        proc.returncode if proc.returncode is not None else "unknown"
                    )
                    # Success = exported OR played. When export dir is used and file was written, that IS success.
                    if player.last_export_path and player.last_export_path.exists():
                        update_status(
                            f"📁 Segment exported to {player.last_export_path.name} (play on host or use Speaker Studio)"
                        )
                    elif _is_success_exit_code(exit_code):
                        status = f"▶️ Played ({seg.start:.1f}s - {seg.end:.1f}s)"
                        update_status(status)
                    else:
                ...
                # No process was created (e.g. Docker without audio device). Success = exported OR played.
                if player.last_export_path and player.last_export_path.exists():
                    update_status(
                        f"📁 Segment exported to {player.last_export_path.name} (play on host or use Speaker Studio)"
                    )
