...
def _apply_speaker_map_to_data(
    ...
    *,
    ...
    """
    Mutate transcript data dict with speaker_map, ignored_speakers, segment updates,
    and canonical schema_version + provenance. Used by update_transcript_json_with_speaker_names
    and by SpeakerMappingService for atomic writes.
    """
    ...
    _method = method if method in _SPEAKER_MAP_METHODS else "interactive"
    data["speaker_map_provenance"] = {
        "tool": "transcriptx",
...
def _update_transcript_json_with_speaker_names(
    transcript_path: str,
    speaker_map: Dict[str, str],
    speaker_id_to_db_id: Optional[Dict[str, int]] = None,
    ignored_speakers: Optional[List[str]] = None,
    ...
    """
    [Private] Update transcript JSON with speaker_map. Prefer SpeakerMappingService for all writes.
    ...
    method must be one of "interactive", "web", "batch" (for speaker_map_provenance).

    Deprecated: Direct use is deprecated (v0.42). Use SpeakerMappingService for all
    mapping writes. This helper may be removed in a future release.
    """
    import warnings

    warnings.warn(
        "Direct use of _update_transcript_json_with_speaker_names is deprecated; "
        "use SpeakerMappingService for all transcript mapping writes.",
        DeprecationWarning,
        stacklevel=2,
    )
    try:
