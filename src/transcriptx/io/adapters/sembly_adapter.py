...
_SEMBLY_EXPORT_MARKERS = (
    b"prodeusembly",
    b"eusembly",
    b"transcription export",
)
_SPEAKER_TS_RE = re.compile(
    r"^\s*(?P<speaker>.+?)\s*[•\-\u2022]\s*(?P<ts>\d{1,2}:\d{2}(?::\d{2})?)\s*$"
)
...
class SemblyAdapter:
    ...
    def _detect_html(self, content: bytes) -> float:
        ...
        if any(marker in snippet for marker in _SEMBLY_EXPORT_MARKERS):
            return 0.8
        return 0.0
    ...
    def _parse_html(self, path: Path, content: bytes) -> IntermediateTranscript:
        ...
        if items:
            turns = self._parse_html_transcript_items(items, warnings)
        else:
            turns = self._parse_html_export_paragraphs(soup, warnings)
            if not turns:
                warnings.append(
                    "No .transcript-item elements or speaker/time paragraph pairs found in Sembly HTML"
                )
    ...
    def _parse_html_transcript_items(
    ...
    def _parse_html_export_paragraphs(
        self, soup: Any, warnings: List[str]
    ) -> List[IntermediateTurn]:
        turns: List[IntermediateTurn] = []
        ...
        while idx < len(p_tags) - 1:
            ...
            if header_p.find("strong") is None:
                idx += 1
                continue

            header_text = header_p.get_text(" ", strip=True)
            match = _SPEAKER_TS_RE.match(header_text)
            if not match:
                idx += 1
                continue

            speaker = match.group("speaker").strip()
            start = self._parse_clock_timestamp(match.group("ts"))
            text = text_p.get_text(" ", strip=True)
            if not text:
                warnings.append(f"Turn {turn_index}: empty text, skipped")
                idx += 2
                continue
    ...
    def _parse_clock_timestamp(self, ts: str) -> Optional[float]:
