...
def validate_schema_version(data: dict) -> bool:
    """Check transcript ``schema_version`` is supported (required key).
...
def validate_segment(segment: Dict[str, Any], index: int) -> None:
    ...
    if not isinstance(segment["speaker"], str):
        raise ValueError(
            f"Segment {index} 'speaker' must be a non-empty string "
            "(canonical v1.0 requires a structural speaker label; use import boundary placeholders if needed)"
        )
    if not segment["speaker"].strip():
        raise ValueError(
            f"Segment {index} 'speaker' must be non-empty "
            "(canonical v1.0 requires a structural speaker label)"
        )
