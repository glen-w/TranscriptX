    ...
    if outputs_dir.exists():
        for speaker_map_file in outputs_dir.rglob("*_speaker_map.json"):
            ...
            if transcript_path:
                ...
                logger.warning(
                    f"Could not find transcript for speaker map: {speaker_map_file}"
                )
        ...
        for speaker_id, speaker_name in normalized_map.items():
            try:
                ...
                if existing_speakers:
                    ...
                    if verbose:
                        print(
                            f"   🔗 Linked '{speaker_name}' to existing speaker (ID: {selected_speaker.id})"
                        )
                else:
                    # Create new speaker
                    if not dry_run:
                        display_name = format_speaker_display_name(
                            first_name=first_name, surname=surname, name=speaker_name
                        )
                        ...
                        if verbose:
                            print(
                                f"   ✅ Created speaker: {speaker_name} (ID: {speaker.id})"
                            )
                    else:
                        results["speakers_created"] += 1
                        if verbose:
        ...
        if not dry_run:
            try:
                update_transcript_json_with_speaker_names(
                    transcript_path, normalized_map, speaker_id_to_db_id
                )
                results["transcript_updated"] = True

                if verbose:
                    print("   ✅ Updated transcript JSON with speaker names")
            except Exception as e:
            ...
            results["transcript_updated"] = True
            if verbose:
                print("   [DRY RUN] Would update transcript JSON")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate existing speaker maps to new format"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--transcript", help="Migrate specific transcript (by base name)"
    )
    ...
    if args.transcript:
        speaker_maps = [
            (sm_path, t_path)
            for sm_path, t_path in speaker_maps
