...
def _check_untracked_forbidden(globs: list[str]) -> list[str]:
    """Fail when forbidden paths exist on disk and are not gitignored (or are setup.py)."""
    hits: list[str] = []
    for glob in globs:
        ...
        if not candidate.exists():
            continue
        # setup.py must not exist at all
        if glob == "src/setup.py":
            hits.append(glob)
            continue
        # Otherwise only fail if git sees it as untracked (not ignored)
        proc = subprocess.run(
...
def _check_ignored_forbidden(globs: list[str]) -> list[str]:
    hits: list[str] = []
    for glob in globs:
        # Use git check-ignore / filesystem walk for concrete prefixes
...
def main() -> int:
    ...
    # data/perf on disk is allowed locally (gitignored) but release-checks may scan;
    # Wave 0: fail only if tracked (already covered). Soft-warn if present as ignored
    # with content during release-checks is optional — plan says "separate scan for
    # ignored but forbidden local-output paths". Treat as failure when present.
    ignored_hits = _check_ignored_forbidden(ignored_globs)
    if ignored_hits:
        # Local perf telemetry is expected during development; only fail when CI
        # sets TRANSCRIPTX_STRICT_IGNORED_FORBIDDEN=1
        import os

        if os.environ.get("TRANSCRIPTX_STRICT_IGNORED_FORBIDDEN", "0") == "1":
            fail = 1
            ...
            print(
                ...
                f"({len(ignored_hits)}); set TRANSCRIPTX_STRICT_IGNORED_FORBIDDEN=1 to fail"
            )
