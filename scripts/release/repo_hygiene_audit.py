...
def check_owner_paths() -> list[str]:
    ...
    # Real home paths only — ignore ellipsis placeholders like /Users/...
...
def check_archive_banners() -> list[str]:
    ...
    for rel in _tracked(["archive/scripts/**"]):
        ...
        if not path.is_file():
            continue
        text = path.read_text(encoding="utf-8", errors="replace")[:800]
        if "[ARCHIVED]" not in text and "Archived" not in text:
            warns.append(f"script archive missing [ARCHIVED] marker: {rel}")
    return warns
