...
class TestCoverageAnalyzer:
    ...
    def analyze_ast_tree(self, tree: ast.AST, file_path: Path) -> Dict:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                module_info['imports'].append(f"{node.module}.{', '.join(n.name for n in node.names)}")
    ...
    def identify_coverage_gaps(self, source_modules: Dict, test_modules: Dict) -> Dict:
        ...
        for module_path in source_modules['modules']:
            module_name = f"transcriptx.{module_path.replace('/', '.').replace('.py', '')}"
        ...
        gaps['coverage_metrics'] = {
            'module_coverage': (tested_modules_count / total_modules) * 100 if total_modules > 0 else 0,
            'function_coverage': ((len(source_modules['functions']) - len(gaps['untested_functions'])) / len(source_modules['functions'])) * 100 if source_modules['functions'] else 0,
            'class_coverage': ((len(source_modules['classes']) - len(gaps['untested_classes'])) / len(source_modules['classes'])) * 100 if source_modules['classes'] else 0
        }
        ...
        print(f"📊 Function coverage: {gaps['coverage_metrics']['function_coverage']:.1f}%")
