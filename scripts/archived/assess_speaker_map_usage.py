...
class SpeakerMapUsageFinder(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        ...
        for alias in node.names:
            if (
                "speaker_map" in alias.name.lower()
                or "speaker_mapping" in alias.name.lower()
            ):
                self.imports.append(
                    {
                        "module": alias.name,
...
def scan_file(file_path: Path) -> Dict[str, Any]:
    ...
    has_usage = (
        bool(regex_matches)
        or bool(ast_matches["function_params"])
        or bool(ast_matches["imports"])
        or bool(ast_matches["usages"])
    )
...
def generate_report(results: List[Dict[str, Any]]) -> str:
    ...
    for priority in ["HIGH", "MEDIUM", "LOW"]:
        if by_priority[priority]:
            report_lines.append(
                f"### {priority} Priority ({len(by_priority[priority])} files)"
            )
            report_lines.append("")
        ...
        for result in sorted(files, key=lambda x: x["file"]):
            ...
            if result["ast_matches"]["function_params"]:
                ...
                for param in result["ast_matches"]["function_params"]:
                    report_lines.append(
                        f"- `{param['function']}()` accepts `speaker_map` parameter (line {param['line']})"
                    )
                report_lines.append("")
            ...
            if result["ast_matches"]["imports"]:
                ...
                for imp in result["ast_matches"]["imports"]:
                    if "module" in imp:
                        report_lines.append(
                            f"- Import from `{imp['module']}` (line {imp['line']})"
                        )
                    elif "name" in imp:
                        report_lines.append(
                            f"- Import `{imp['name']}` from `{imp['module']}` (line {imp['line']})"
                        )
                report_lines.append("")
            ...
            if result["regex_matches"]:
                ...
                for pattern_name, matches in result["regex_matches"].items():
                    report_lines.append(
                        f"- {pattern_name}: {len(matches)} occurrence(s)"
                    )
                    for match in matches[:5]:  # Show first 5
                        report_lines.append(
                            f"  - Line {match['line']}: {match['match']}"
                        )
                    if len(matches) > 5:
            ...
            if result["ast_matches"]["usages"]:
                ...
                for usage in result["ast_matches"]["usages"][:10]:  # Show first 10
                    func_context = (
                        f" in `{usage['function']}()`" if usage.get("function") else ""
                    )
                    report_lines.append(
                        f"- {usage['type']}: `{usage.get('name', 'N/A')}` (line {usage['line']}){func_context}"
                    )
...
def main():
    ...
    # Save report
    report_path = (
        project_root / "docs" / "development" / "SPEAKER_MAP_ASSESSMENT_REPORT.md"
    )
    ...
    json_path = (
        project_root / "docs" / "development" / "SPEAKER_MAP_ASSESSMENT_DATA.json"
    )
