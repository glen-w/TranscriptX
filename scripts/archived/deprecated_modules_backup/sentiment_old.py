...
from transcriptx.core.utils.output_standards import create_summary_json
...
class SentimentAnalysis(AnalysisModule):
    ...
    def analyze(self, segments: List[Dict[str, Any]], speaker_map: Dict[str, str] = None) -> Dict[str, Any]:
        """
        ...
        """
        from transcriptx.core.utils.speaker_extraction import extract_speaker_info, get_speaker_display_name
        ...
        for speaker, segs in speaker_segments.items():
            ...
            speaker_stats[speaker] = {
                "count": len(scores),
                "compound_mean": sum(s.get("compound", 0) for s in scores) / len(scores),
        ...
        if all_rows:
            global_stats = {
                "count": len(all_rows),
                "compound_mean": sum(r["compound"] for r in all_rows if r["compound"] is not None) / len(all_rows),
                "pos_mean": sum(r["pos"] for r in all_rows if r["pos"] is not None) / len(all_rows),
                "neu_mean": sum(r["neu"] for r in all_rows if r["neu"] is not None) / len(all_rows),
                "neg_mean": sum(r["neg"] for r in all_rows if r["neg"] is not None) / len(all_rows),
            }
        else:
            global_stats = {}
    ...
    def _save_results(self, results: Dict[str, Any], output_service: 'OutputService') -> None:
        """
        ...
        enriched_path = get_enriched_transcript_path(
            output_service.transcript_path, 
            "sentiment"
        )
        ...
        for speaker, segs in speaker_segments.items():
            ...
            output_service.save_chart(fig, f"{speaker}_rolling_sentiment", speaker=speaker)
    ...
    def _create_multi_speaker_plot(self, segments: List[Dict], speaker_map: Dict[str, str]):
