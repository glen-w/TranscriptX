...
def extract_audio_properties(ffprobe_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if audio_stream:
        ...
        props["bit_depth"] = audio_stream.get("bits_per_sample") or audio_stream.get(
            "bits_per_raw_sample"
        )
...
def analyze_audio_quality(audio_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics["clipping_detected"] = clipped_samples > (
            len(samples) * 0.001
        )  # >0.1% clipped
    ...
    try:
        ...
        if len(speech_freqs) > 0:
            ...
            if freq_analysis["frequency_centroid"]:
                variance = np.sum(
                    ((speech_freqs - freq_analysis["frequency_centroid"]) ** 2)
                    * speech_mags
                ) / np.sum(speech_mags)
...
def calculate_compression_efficiency(
    file_size: int, duration: float, bitrate: int
) -> Dict[str, Any]:
...
def compare_audio_files(file1_path: Path, file2_path: Path) -> None:
    ...
    for label, file_path in files:
        ...
        print(
            f"  Bitrate:         {props['bitrate']:,} bps ({props['bitrate']/1000:.1f} kbps)"
            if props["bitrate"]
        ...
        if quality["rms_level"] is not None:
            ...
            if quality["clipping_detected"]:
                print(
                    f"  ⚠️  Clipping:      {quality['clipping_percentage']:.2f}% (DETECTED)"
                )
            else:
                print(
                    f"  Clipping:         {quality['clipping_percentage']:.2f}% (none detected)"
                )
        ...
        if freq["dominant_frequencies"]:
            print(
                f"  Dominant Frequencies: {[f'{f:.1f} Hz' for f in freq['dominant_frequencies']]}"
            )
    ...
    if len(all_results) == 2:
        ...
        if r1["props"]["sample_rate"] and r2["props"]["sample_rate"]:
            ...
            print(
                f"  Sample Rate Difference: {abs(sr_diff):,} Hz ({'+' if sr_diff > 0 else ''}{sr_diff:,} Hz)"
            )
            if abs(sr_diff) > 8000:
                print(
                    "    ⚠️  Significant difference - may affect transcription quality"
                )

        if r1["props"]["duration"] and r2["props"]["duration"]:
            dur_diff = r1["props"]["duration"] - r2["props"]["duration"]
            print(
                f"  Duration Difference: {abs(dur_diff):.1f}s ({'+' if dur_diff > 0 else ''}{dur_diff:.1f}s)"
            )
        ...
        if r1["props"]["file_size"] and r2["props"]["file_size"]:
            ...
            print(
                f"  File Size Difference: {format_file_size(abs(size_diff))} ({'+' if size_diff > 0 else ''}{size_diff_pct:.1f}%)"
            )
        ...
        if r1["quality"]["rms_db"] and r2["quality"]["rms_db"]:
            ...
            print(
                f"  RMS Level Difference: {abs(rms_diff):.2f} dB ({'+' if rms_diff > 0 else ''}{rms_diff:.2f} dB)"
            )
            if abs(rms_diff) > 3:
        ...
        if r1["quality"]["silence_percentage"] and r2["quality"]["silence_percentage"]:
            silence_diff = (
                r1["quality"]["silence_percentage"]
                - r2["quality"]["silence_percentage"]
            )
        ...
        if (
            r1["frequency"]["frequency_centroid"]
            and r2["frequency"]["frequency_centroid"]
        ):
            fc_diff = (
                r1["frequency"]["frequency_centroid"]
                - r2["frequency"]["frequency_centroid"]
            )
        ...
        if r1["props"]["sample_rate"] and r1["props"]["sample_rate"] > 16000:
            recommendations.append(
                "ℹ️  File 1 sample rate >16kHz - consider downsampling to 16kHz for transcription (saves space)"
            )

        if r2["props"]["sample_rate"] and r2["props"]["sample_rate"] > 16000:
            recommendations.append(
                "ℹ️  File 2 sample rate >16kHz - consider downsampling to 16kHz for transcription (saves space)"
            )
        ...
        if (
            ...
            recommendations.append(
                "ℹ️  Normalize audio levels for consistent transcription quality"
            )
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("\nExample:")
        print(
            "  python compare_audio_quality.py data/recordings/260108_CSE_pen.mp3 data/recordings/260108_CSE.mp3"
        )
        sys.exit(1)
